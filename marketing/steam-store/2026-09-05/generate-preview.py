...
def main() -> None:
    ...
    assert len(media['screenshots']) == 8 and len(loops) == 4
    ...
    for section, banner in zip(copy['sections'], banner_manifest['outputs']):
        ...
        for file in section['media']:
            ...
            section_html.append(
                f'<video autoplay muted loop playsinline controls preload="metadata" width="1170" height="658" aria-label="{html.escape(loop["alt"])}">'
...
video{display:block;width:100%;height:auto;aspect-ratio:1170/658;background:#000;margin:0 0 20px}
...
'''
    ...
    plan = {
        ...
        'final_ui_checks': ['All eight gallery frames are in the specified order.', 'All four banners and four loops load in the English About This Game section.', 'Every media item has its supplied alt text; banner heading images use the English localization group.', 'No {{STEAM_*}} placeholders remain in the final saved description.', 'Steam preview plays the animations silently and retains expected color, framing and mobile readability.'],
    }
    (ROOT / 'upload-plan.json').write_text(json.dumps(plan, indent=2, ensure_ascii=False) + '\n')
    print(json.dumps({'gallery': 8, 'banners': 4, 'loops': 4, 'description_asset_bytes': total_bytes, 'short_description_characters': len(copy['short_description'])}))
