...
9. Build useful ground deliberately. Fire deals one damage on each entry and two at an actor's start; Detonate consumes selected Fire for one shared union blast. Rubble costs two movement to enter, with one minimum-progress step from a fresh allowance. Ice entry or start activates Chilled (+1 direct damage); a direct Ice hit freezes and consumes all supporting Ice. Electrified connects ordinary Lightning through cardinal components; Chain may use individual tiles to bridge gaps. Prefer mixed setup/payoff that improves this board rather than painting distant empty corners. Ground never grants Light on its own.
