...
10. For first-boss playtests, treat every campfire as a continue point, not a run endpoint. Use `linger` to heal and continue when available; use `level STAT STAT` only when the stat gain clearly improves boss odds. Do not use `rest` or stop the run at a campfire unless the harness is blocked and no legal progress is possible.
