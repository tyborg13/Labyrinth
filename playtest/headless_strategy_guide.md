...
10. For first-boss playtests, treat every campfire as a continue point, not a run endpoint. Use `linger` to heal and continue when available; use `level SKILL_ID` when one of the legal abilities strengthens the current plan. The console prints each legal ID and its effect. Do not use `rest` or stop the run at a campfire unless the harness is blocked and no legal progress is possible.
