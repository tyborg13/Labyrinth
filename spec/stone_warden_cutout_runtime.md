...
Fresh evidence is in [runtime_v2](../experiments/cutouts/stone_warden/runtime_v2/):
...
- `gameplay/warden_gameplay_full_speed.mp4`: **33.80 seconds**, 1920×1080/60 fps, covering all four real Marching Blow directions, Crushing Step, reduced motion, Bulwark, death and player movement around all four sides of a Warden. The twelve source clips and 878 recorded samples retain actual wall-clock intervals; encoding repeats frames without inventing poses or changing speed. The complete reel decodes without errors. `video_timeline.json` records per-frame durations and rounding, at most one 60 fps frame per clip.
- `gameplay/player_reposition_facing.mp4`: **5.50 seconds** focused on the four real two-tile player moves. In every native sample, idle facing stays fixed while the player is walking, then changes toward the completed destination. Both Wardens face the player independently; the protagonist still returns to front/unmirrored idle. The four final-state PNGs show each direction.
...
- 33 native 1920×1080 PNGs and five mid-step JPEGs retain the inspected states. Both native runner manifests are accepted on macOS Metal/mobile.
- `workflow_validation.log`: both edited skills pass their standard validator. The enemy skill links to the maintained cutout integration policy, which names the shared facing helper and the four-side movement proof.
- `focused_suite.log`: **PASS**, including player-facing idle after actions/repositioning, reduced-motion facing, frozen death direction, independent actors, four facings, distance phase wrap, attack/contact/trail timing, production speed ratio, reduced motion, hidden updates, stable logical registration, destination echoes, death geometry and renderer release.
...
- `capture_input_sha256.json` binds 1,470 case/runtime/art/test/import/export/workflow inputs to the capture and checks. `proof_sha256.json` binds all 71 retained outputs. No bound input changed during or after capture. The old v03 board proof and runtime_v1 integration proof are historical because their shared runtime dependency hashes changed; this new direct native comparison proves the approved poses remain intact.
...
The self-healing fixture command regenerates and independently reloads the pre-action save before opening the game. Choose **Continue**, select the player, then choose a nearby destination to see idle facing update after the move. Choose **Pass** to see the two Wardens advance and attack. Moving around them shows the front/rear/reflected views. Brace and Patch Up allow further turns; the fixture starts the player at 40 HP. The review branch is `codex/stone-warden-editable-front-rear-cutout`, based on local `master` commit `65b667daf19568ff62892b458e36a41645f579be`.

```sh
cd /Users/borgerding/workspace/Labyrinth.worktrees/stone-warden-editable-front-rear-cutout && python3 tools/inspection_fixture.py --task-id stone-warden-editable-front-rear-cutout --run-id stone-warden-runtime-v2-inspection --manifest /private/tmp/stone-warden-runtime-v2-inspection.json --launch --scenario combat --summary "Stone Warden cutout: move the player to see idle facing, then Pass for movement and overhead strikes" --player-position 3:5 --player-hp 40 --player-max-hp 40 --enemy-types warden,warden --enemy-positions 3:3,5:5 --enemy-intents marching_blow,crushing_step --hand quick_stab,brace,sidestep_slash,bone_dart,patch_up
```
