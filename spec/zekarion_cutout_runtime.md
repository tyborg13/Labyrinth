...
Each view has 22 bones: root/torso, neck/head/jaw, two wings, paired upper/lower forelegs and claws, paired thighs/shins/feet, and a three-part tail. Ten weighted meshes blend limb/neck/wing/tail roots and the jaw hinge; claws, feet and head remain rigid terminal paint. The jaw and its existing concealed cap share a head-to-jaw field: front center (85,126), axis (-1,0), 14px width; rear center (174,81), axis (1,0), 8px width. Both use 1px mesh spacing. The proximal cheek stays attached to the skull while the distal jaw opens, removing the diagonal split previously exposed by Tempest Breath. This changes mesh ownership only; all original PNGs and the jaw phase/rotation remain unchanged. Proximal skin bands blend into the torso/root, and distal bands reach the rigid claw/foot before the visible seam. Small head/neck rotations are coordinated rather than independently bending the head off its collar.
...
Each living actor owns one persistent SubViewport texture and two loaded rigs. All retained board layers, destination echoes, reduced-motion stills and death dissolves share that actor's texture. Death retains the current facing and plays the authored forequarter collapse into the existing dissolve; removal releases only that actor. Hit gives a brief shoulder/neck recoil with grounded claws, recovering to rest. See [contextual cutout reactions](contextual_dragon_cutout.md) for the shared presentation boundary and cohort audit. Cached HUD/shadow geometry uses the neutral bake and does not read back the moving viewport each frame.
