...
The final presentation keeps Rubble still, refining the design's optional dust treatment. Its stable rock shapes remain readable underneath moving elemental effects and distinguish difficult terrain from active hazards. The elemental layers retain their animated loops; Rubble's static identity also permits retaining its geometry without recurring draw work.
...
python3 tools/visual_probe_runner.py tests/board_surface_visual_probe.gd --project . --task-id board-surface-refactor --no-headless --expect-size 1920x1080 --min-images 19 --result-manifest output/board-surface-refactor/visual-proof-final.json
