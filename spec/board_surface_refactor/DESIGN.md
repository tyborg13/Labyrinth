# Board surfaces and the complete card refactor — implementation contract v5
...
- Electrified has two uses: ordinary damaging Lightning conducts through contiguous patches and strikes their occupants; Chain can also use individual Electrified tiles as relay nodes across its normal jumps. Ordinary Electrified remains after use; conductive Fire from Stormcoal is consumed. The surface itself causes no entry damage or Shock.
- Ice activates Chilled on actual entry or the affected unit's turn start. Chilled then persists while occupying Ice. Freeze consumes its supporting Ice. No extra thawing-protection status is needed. Chilled adds +2 per direct hit; Frozen triples direct damage until its skipped activation.
...
Leaving Rubble costs **2 movement instead of 1**. Entering it adds no separate cost. Printed Move and the player's independent movement use the same weighted costs. For a large actor, use the maximum departure cost of its vacated footprint tiles, not the sum.
...
**Accepted tuning:** Chilled adds **+2 damage to each direct damaging attack** while active. The sustained bonus applies to every Flurry/Chain/multi-hit. It does not manufacture damage on support/status actions or amplify passive hazards and damage-over-time. Exact values remain provisional pending whole-pool balance.
...
**Reusable conduction:** activate the whole connected component, including empty connecting tiles and branches. Each eligible actor is hit once. Ordinary Electrified remains for subsequent attacks; Stormcoal Fire in the component is consumed. Disconnected components stay inactive unless another native hit or Chain relay reaches them.
...
## Accepted feedback revision — rules v5

- Fire: 2 damage on entry and 3 at actor start. Chilled: +2 direct damage.
  Frozen: ×3 direct damage until the skipped activation, without refreshing.
  Passive Fire and Bleed receive neither attack multiplier.
- Rubble charges the movement step when a footprint vacates a Rubble tile.
  Fresh-allowance minimum progress remains; forced movement pays no surcharge.
- Electrified persists through both Chain relays and ordinary Lightning
  conduction. `surface_conducted` records use separately from removal.
  Stormcoal Fire remains consumable. Ion Spool rewards conduction, once per turn;
  electrical card/enemy riders require actual assisted hits.
- Printed base card attacks use 80% damage, rounded half up, minimum 1.
  Detonate damage is unchanged. Root Snare keeps Rubble, removes Immobilize,
  and costs 4 Time. Shale Burst leaves Rubble throughout its attack pattern.
- Tooltips and relic descriptions are short reminders, with nested concepts;
  follow the reusable copy rules in COPY.md. Same-area followups show one
  pattern, and conditions use inline surface icons with no intensity block.
