...
| --- | --- | --- | --- |
| Thunder Relay (Epic) | One of 7 accessible Shock sources while raw Lightning is 2+. Most conditional Shock cards raise intensity before applying Shock, so this is normally the first or second Shock card. | Once per turn. Deals 3 to each enemy: 9/12/15 total in 3/4/5-enemy rooms. | Feasible recurring discharge; replaces the unrealistic third-Shock threshold and generic draw. |
...
| Vaulting Sigil (Epic) | A real four-tile resolved path. Twelve cards print Move/Blink 4+, including starter Shadow Step; obstacles can still shorten Move paths. | Once per turn; +1 play and 4 Block. It cannot loop from the same move. | Four is demanding but routinely achievable; six was not. |
...
| Duelist Whetstone (Common) | First card each turn among 21 cards that both Move/Blink and attack. | +2 per affected target on that card only: usually +2; at most +10 across five targets before Freeze. | Offensive identity matches the rapier/whetstone theme and no longer overlaps Pilgrim Boots. |
...
| Ion Spool (Common) | First Shock each turn grants 1 Lightning; crossing 4 consumes 2. From 0–1 starting Lightning, two coordinated Shock cards normally establish the first discharge. | Once per turn; 4 to all enemies = 12/16/20 total. Consumption resets the next setup instead of allowing passive repetition. | Preserves the liked charge/consume engine with a visible electric payoff. |
| Cinderbrand Tongs (Common) | First Burn each turn grants 1 Fire; crossing 4 consumes 2. Thirteen Burn definitions make two-turn setup realistic. | Once per turn; immediate 2 to all plus Burn 1 = 6/8/10 immediate and up to 9/12/15 after the first Burn tick. | Preserves the engine and replaces generic play economy with Fire pressure. |
| Coffin Nails (Common) | Hold Block from any of 41 sources, then attack. This also works with its six equipment-based Bleed cards but does not require owning one. | Adds Bleed 1 to every attack resolved while Block remains; base two plays usually permit one setup card and one payoff card. A five-target AOE can apply five Bleed. | Nail-appropriate Bleed bridge; no Burn-card mismatch. |
...
| Basalt Calendar (Rare) | Count the 12 available Earth cards in the deck. | Start at +1 Earth for 3–5 Earth cards and +2 for 6+; hard cap 2. In an Earth room, six-card commitment starts at 3 total, while smaller investments do not bypass every gate. | Progressive commitment replaces the binary +3 breakpoint. |
...
| Black Sun Dial (Legendary) | All five raw elements at 2. This needs about 9 intensity points in an elemental room or 10 in neutral, versus 19–20 under the old threshold. A five-color deck reaches it in a long normal fight or boss. | Once per combat; consume 2 each, deal 12 to all (36–60 across 3–5 enemies), gain 12 Stoneskin, draw 3, gain 3 plays. | Preserves the hard five-color build-around while making the swing turn both attainable and usable. |
