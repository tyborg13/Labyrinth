...
repos={'candidate':Path('/Users/borgerding/workspace/Labyrinth.worktrees/thorough-steam-deck-performance-pass-across-varied-gameplay-and-animatio'),'base':root/'hitch-baseline'}
...
def snapshot(repo,env):
 files=list(repo.joinpath('scripts').glob('*.gd'))+[repo/'project.godot',repo/'tests/runtime_frame_performance_benchmark.gd',repo/'tests/representative_combat_performance_benchmark.gd',repo/'tools/performance_pass.py',repo/'tools/visual_probe_runner.py']
 return {'head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True).strip(),'environment':env,'files':{str(p.relative_to(repo)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(files) if p.exists()}}
for label,build,bench,extra in items[6:]:
