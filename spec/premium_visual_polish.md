...
## Fourth finish pass: inset health meters

The health meter should answer how much health remains and how a proposed action changes it at a glance. The existing board, terrain and turn-order bars keep their positions, dimensions, segment cadence, exact numeric text, team colors and damage-preview overlays. A shaded well, lit enamel shoulder and crisp depleted endpoint add depth inside the existing geometry. This is a static material change; it adds no motion or input handling and uses the shared `SegmentedHealthBar` renderer. Native normal, damaged, preview and dense-combat captures plus existing health-theme/turn-order tests verify that readability, preview authority and layout are preserved at 1920×1080/UI100. All ten rubric gates apply to this pass; no copy, identity or rule change is proposed.

The same fourth pass adds [shape-matched skill medallions and a committed-learn confirmation](premium_skill_polish.md), plus [loadout contact depth and bounded merchant feedback](premium_loadout_polish.md). These surfaces preserve the existing information layout, semantic state colors and native input paths. The approved pickup, treasure, campfire and character presentation remain outside this pass.
