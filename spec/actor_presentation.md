...
The solver separates scalable body geometry from screen-pixel health bars (142×34 with 10px clearance), then finds the largest tile width inside the available combat rectangle. At 1920×1080 the available rectangle is `(36,12,1848,726)`, reserving the focused hand's vertical extent. The room signature contains fixed grid/room geometry; moving, turning, spawning, hiding, dying, controller focus and reward overlays cannot recalculate its envelope. Explicit navigation zoom/pan and a viewport resize remain supported.
