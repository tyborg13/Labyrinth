...
`BoardFraming` captures fixed room props/floor and reserves **only the current encounter’s possible enemy types** at legal origins, respecting large footprints. The room layout supplies the initial roster, including hidden enemies. RunScene caches this budget by run and room before combat; victory rebuilds the room layout without enemies, so later display layouts must never replace that cached roster. Entering a new room captures its own budget. Its recursive closure adds death splits, summon intents and the encounter’s survival reinforcement pool. Unrelated dragons are never budgeted for an ordinary fight. Current actor rectangles and their transparent canvases do not inflate the fixed envelope. Source action bounds include both painted views and reflections, sampled across every configured action, plus an 8px source margin. Future art or motion changes must update these profiles with fresh native evidence.
