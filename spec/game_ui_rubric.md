...
- The authored canvas and default visual-proof target are `1920x1080` at `100%` UI scale. Lower-resolution, handheld, and alternate-scale QA are deferred unless the user or task explicitly targets them. Player UI scale still supports `90%`, `100%`, `115%`, and `125%`, and motion can be reduced.
...
1. Render visual proof at `1920x1080` and `100%` UI scale. This is the only default resolution/scale requirement. Do not add constrained-resolution, handheld, or alternate-scale runs unless the user or task explicitly requests them.
...
5. Exercise every input path the changed surface currently supports. Verify pointer behavior, keyboard focus/order, and controller focus traversal, activation, back/cancel, and focus recovery as applicable. Verify input-device handoff and displayed cues when an active-input system exists. Also verify reduced-motion behavior for new motion; input coverage does not imply extra resolution runs unless the task explicitly requests them.
