...
const ProductionRig = preload("res://scripts/frostglass_lancer_cutout/rig.gd")
const AcceptedMotion = preload("res://experiments/cutouts/frostglass_lancer/v01/motion.gd")
const CaseRig = preload("res://tools/cutout_pipeline/rig.gd")
const OUTPUT: String = "user://probes/frostglass_lancer_cutout_assets"
...
func _run() -> void:
	...
	for facing: String in ["front", "rear"]:
		...
		_check(reference.configure("res://experiments/cutouts/frostglass_lancer/v01/cutout.json"), "Reviewed case loads")
		...
		for clip: String in ["rest", "idle", "walk", "thrust", "cast", "pin"]:
			var frames: int = 1 if clip == "rest" else 24 if clip == "idle" else 32
			for index: int in range(frames):
				var phase: float = float(index) / float(frames - 1 if clip in ["thrust", "cast", "pin"] else frames)
				if clip in ["walk", "thrust", "cast", "pin"]:
					_check(ProductionRig.FrostglassMotion.sample_pose(clip, phase, production.layout, facing) == AcceptedMotion.sample_pose(clip, phase, production.layout, facing), "Accepted walk/attack transforms remain identical")
				...
				compared += 1
				if clip in ["thrust", "cast", "pin"] and index in [10, 13, 17, 21]:
					image.save_png(OUTPUT.path_join("%s_%s_%02d.png" % [facing, clip, index]))
				if clip == "rest":
					var baked: Image = Image.load_from_file("res://assets/units/frostglass_lancer_cutout/" + facing + "/rest.png")
	...
	print("FROSTGLASS ASSET PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
