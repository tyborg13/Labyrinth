...
const OUTPUT: String = "user://probes/cinder_ooze_gameplay_v1"
...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://cinder_ooze_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://cinder_ooze_probe_run.save")
	...
	for index: int in range(directions.size()):
		...
		_assert(_texture_id != 0, "Actual RunScene owns the Cinder Ooze texture")
		...
		await _record("%02d_smolder_%s" % [index + 1, _direction_name(index)], false, true)
		var after: Dictionary = _instance.get("_combat_state")
		_record_outcome("smolder_" + _direction_name(index), expected)
		_assert(int(after["player"]["hp"]) == int(expected["player"]["hp"]), "Smolder Slide and its Fire timing exactly match the unanimated resolver")
		_assert(after["enemies"][0]["pos"] == origin + directions[index] and after["enemies"][0]["pos"] == expected["enemies"][0]["pos"], "Smolder Slide follows the resolved one-tile route")
	...
	for index: int in range(directions.size()):
		await _fixture(Vector2i(3, 3) + directions[index], Vector2i(3, 3), false, "cinder_bloom")
		_texture_id = int(_snapshot()["texture_id"])
		var expected: Dictionary = _expected_after_pass()
		_instance.call("_on_pass_turn_pressed")
		await _record("05_bloom_" + _direction_name(index), false, false)
		_assert((_manifest["clips"][-1]["phases_seen"] as Array).has("bloom"), "End Turn triggers the distinct outward eruption")
		_record_outcome("bloom_" + _direction_name(index), expected)
		_assert(_instance.get("_combat_state") == expected, "Cinder Bloom preserves the complete area/fire/status/initiative outcome")
	for action: String in ["smolder_slide", "cinder_bloom"]:
		await _fixture(Vector2i(3, 4), Vector2i(3, 3), true, action)
		_texture_id = int(_snapshot()["texture_id"])
		var expected: Dictionary = _expected_after_pass()
		_instance.call("_on_pass_turn_pressed")
		await _record("06_reduced_" + action, false, false)
		_record_outcome("reduced_" + action, expected)
		_assert(_instance.get("_combat_state") == expected, "Reduced motion preserves the same complete outcome")
		_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the Cinder Ooze still")
	for fueled: bool in [false, true]:
		await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "slag_shell", 12, fueled)
		_texture_id = int(_snapshot()["texture_id"])
		var expected: Dictionary = _expected_after_pass()
		_instance.call("_on_pass_turn_pressed")
		await _record("07_slag_shell_" + ("fueled" if fueled else "plain"), false, false)
		_record_outcome("slag_shell_" + ("fueled" if fueled else "plain"), expected)
		_assert(_instance.get("_combat_state") == expected, "Slag Shell preserves block and fuel consumption")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "smolder_slide", 9)
	_texture_id = int(_snapshot()["texture_id"])
	...
	await _record("09_cinder_ooze_death", true, false)
	_assert(_snapshot().is_empty(), "Dead Cinder Ooze releases its cutout after the dissolve")
	...
	_assert(survivors.size() == 3, "Lethal hit preserves the other Ooze and creates two split children")
	var children: Array = survivors.filter(func(enemy: Dictionary) -> bool: return str(enemy.get("type", "")) == "cinder_droplet")
	_assert(children.size() == 2 and children[0]["summoned"] and children[1]["summoned"], "Both split children remain summoned droplets")
	_manifest["split_outcome"] = {"survivors": survivors.duplicate(true), "room_embers": (_instance.get("_combat_state") as Dictionary).get("room_embers", 0)}
	_assert(int((_instance.get("_combat_state") as Dictionary).get("room_embers", 0)) == 12, "Original Ooze reward is paid exactly once")
	_assert((_board.get("_cinder_ooze_renderers") as Dictionary).size() == 1, "Split droplets do not inherit the parent's cutout or motion")
	...
	for index: int in range(around.size()):
		...
		_assert(_snapshot()["clip"] == "idle" and _snapshot()["facing"] == facing["facing"] and _snapshot()["mirrored"] == facing["mirrored"], "Cinder Ooze idle turns toward the player's completed destination")
	...
	var portrait: String = str(_instance.TURN_ORDER_PORTRAITS.get("cinder_ooze", ""))
	_assert(portrait == "res://assets/art/portraits/cinder_ooze.png" and preload("res://scripts/asset_loader.gd").load_texture_source_first(portrait) != null, "Turn clock retains the dedicated Cinder Ooze portrait")
	...
	print("CINDER OOZE GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "smolder_slide", cinder_ooze_hp: int = 12, fueled: bool = false) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Cinder Ooze Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "cinder_ooze", "pos": enemy_tile, "hp": cinder_ooze_hp, "max_hp": 12, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "cinder_ooze", "pos": Vector2i(6, 6), "hp": 12, "max_hp": 12, "block": 0})
	...
	for intent: Dictionary in GameData.enemy_def("cinder_ooze")["intents"]:
	...
	if fueled:
		preload("res://scripts/board_surface_rules.gd").place(state, enemy_tile + Vector2i(-1,0), "fire")
...
func _expected_after_pass() -> Dictionary:
	var engine := CombatEngine.new()
	var state: Dictionary = (_instance.get("_combat_state") as Dictionary).duplicate(true)
	return engine.advance_to_next_player_turn_with_steps(engine.finish_player_activation(state))["state"]
...
func _record(label: String, allow_death: bool, require_attack: bool, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		_assert((allow_death and snapshot.is_empty()) or int(snapshot.get("texture_id", 0)) == _texture_id, "One Cinder Ooze texture remains live through " + label)
		...
		if not observer_before.is_empty() and str(player_motion.get("clip", "")) == "walk":
			...
			_assert(snapshot["facing"] == observer_before["facing"] and snapshot["mirrored"] == observer_before["mirrored"], "Cinder Ooze waits until player movement finishes before turning")
		...
		for candidate: Dictionary in _board.call("_visible_units"):
			if str(candidate.get("key", "")) == "enemy_1":
				unit = candidate
		if str(snapshot.get("clip", "")) in ["attack", "bloom"]:
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Cinder Ooze attack keeps its planted stance on its actual tile")
		...
		if str(snapshot.get("clip", "")) == "walk" and not unit.is_empty():
			...
			if previous_support.get("view", "") == support["view"]:
				for lobe: String in Cutout.Motion.CONTACT_NAMES:
					var foot: String = "contact_" + lobe
	...
	if require_attack:
		_assert(phases.has("walk") and phases.has("attack"), "Actual End Turn plays the Cinder Ooze walk and attack in " + label)
	...
	if not images.is_empty():
		...
		for target: float in [0.25, 0.38, 0.42, 0.65]:
			var nearest: int = -1
			var difference: float = INF
			for index: int in range(samples.size()):
				if str(samples[index]["animation"].get("clip", "")) not in ["attack", "bloom"]:
					continue
...
func _support_points(unit: Dictionary, snapshot: Dictionary) -> Dictionary:
	var renderer: Node = (_board.get("_cinder_ooze_renderers") as Dictionary)["enemy_1"]
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "attack"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Cinder Ooze movement and melee face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("cinder_ooze_animation_snapshot", "enemy_1")
...
func _record_outcome(label: String, expected: Dictionary) -> void:
	var actual: Dictionary = _instance.get("_combat_state")
	_assert(actual == expected, label + " preserves the complete resolver outcome")
	_manifest["outcomes"].append({"label": label, "identical": actual == expected, "actual": actual.duplicate(true), "expected": expected.duplicate(true)})
