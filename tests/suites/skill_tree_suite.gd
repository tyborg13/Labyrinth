...
static func _test_skill_data_and_topology(expect: Callable) -> void:
	...
	expect.call(SkillTreeLibrary.description("ghost_stride") == "Once per combat, your free @icon(move) 2 becomes @icon(blink) 2.", "Ghost Stride copy should clearly transform the activation's free movement")
	...
	expect.call(SkillTreeLibrary.description("prismatic_instinct") == "Once per combat, name a card in hand that creates a Field or Surface. The next printed play of any copy extends those tiles by 6 @icon(time).", "Prismatic Instinct copy should describe its card-name scope and tile-duration payoff")
	...
	expect.call(SkillTreeLibrary.description("confluence") == "The first time each turn you place a Surface on a Field, @icon(draw) 1.", "Confluence copy should explain its simple cross-layer board payoff")
