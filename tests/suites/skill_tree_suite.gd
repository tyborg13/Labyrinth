...
static func _test_skill_data_and_topology(expect: Callable) -> void:
	...
	expect.call(SkillTreeLibrary.description("living_shadow") == "Once between your turns, when an illusion is destroyed or dispelled, return your most recently discarded non-item card to hand—or put it atop your draw pile if your hand is full.", "Living Shadow copy should use turn cadence, identify both illusion-removal triggers, and define a full hand")
