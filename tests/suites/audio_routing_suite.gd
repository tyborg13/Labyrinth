...
static func run(expect: Callable) -> void:
	...
	expect.call(_reverb_count(music_index) == 1, "Music should contain exactly one restrained room reverb")
	...
	quiet_settings["music_volume"] = 0.37
	...
	var expected_music_db: float = linear_to_db(0.37)
	...
	expect.call(is_equal_approx(AudioServer.get_bus_volume_db(music_index), expected_music_db), "Music reverb should preserve the independently controlled player music volume")
	var muted_settings: Dictionary = quiet_settings.duplicate(true)
	muted_settings["sfx_volume"] = 0.0
	muted_settings["music_volume"] = 0.0
	...
	expect.call(AudioServer.is_bus_mute(music_index), "Muting music should silence its direct signal and reverb together")
	SettingsStore.apply_audio_settings(SettingsStore.default_settings())

	var world_reverb_index: int = _reverb_index(world_index)
	if world_reverb_index >= 0:
		var world_reverb := AudioServer.get_bus_effect(world_index, world_reverb_index) as AudioEffectReverb
		expect.call(AudioServer.is_bus_effect_enabled(world_index, world_reverb_index), "World room reverb should be enabled")
		expect.call(is_equal_approx(world_reverb.dry, SettingsStore.WORLD_REVERB_DRY), "World room reverb should retain the full direct signal")
		expect.call(is_equal_approx(world_reverb.wet, SettingsStore.WORLD_REVERB_WET), "World room reverb should use the restrained wet mix")
		expect.call(world_reverb.wet <= 0.15, "World room reverb should stay below an overtly wet combat mix")
		expect.call(is_equal_approx(world_reverb.predelay_msec, SettingsStore.WORLD_REVERB_PREDELAY_MSEC), "World room reverb should preserve transient clarity with short pre-delay")
		expect.call(is_equal_approx(world_reverb.room_size, SettingsStore.WORLD_REVERB_ROOM_SIZE), "World room reverb should use the authored chamber size")
		expect.call(is_equal_approx(world_reverb.damping, SettingsStore.WORLD_REVERB_DAMPING), "World room reverb should use the authored dark damping")
		expect.call(is_equal_approx(world_reverb.hipass, SettingsStore.WORLD_REVERB_HIPASS), "World room reverb should trim low-frequency buildup")

	var music_reverb_index: int = _reverb_index(music_index)
	if music_reverb_index >= 0:
		var music_reverb := AudioServer.get_bus_effect(music_index, music_reverb_index) as AudioEffectReverb
		expect.call(AudioServer.is_bus_effect_enabled(music_index, music_reverb_index), "Music room reverb should be enabled")
		expect.call(is_equal_approx(music_reverb.dry, SettingsStore.MUSIC_REVERB_DRY), "Music room reverb should retain the full mastered signal")
		expect.call(is_equal_approx(music_reverb.wet, SettingsStore.MUSIC_REVERB_WET), "Music room reverb should use its independently authored wet mix")
		expect.call(music_reverb.wet < SettingsStore.WORLD_REVERB_WET, "Music reverb should remain lighter than the world SFX treatment")
		expect.call(is_equal_approx(music_reverb.predelay_msec, SettingsStore.MUSIC_REVERB_PREDELAY_MSEC), "Music room reverb should use its authored short pre-delay")
		expect.call(is_equal_approx(music_reverb.room_size, SettingsStore.MUSIC_REVERB_ROOM_SIZE), "Music room reverb should share the compact chamber scale")
		expect.call(is_equal_approx(music_reverb.damping, SettingsStore.MUSIC_REVERB_DAMPING), "Music room reverb should use the darker authored damping")
		expect.call(is_equal_approx(music_reverb.hipass, SettingsStore.MUSIC_REVERB_HIPASS), "Music room reverb should limit low-frequency buildup")
	...
	expect.call(music_player != null and music_player.bus == SettingsStore.MUSIC_BUS, "Run-scene music should use the independently reverberant Music bus")
	run_scene.free()
