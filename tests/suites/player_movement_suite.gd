...
static func _test_default_pool_and_split_spending(expect: Callable) -> void:
	...
	var invalid_after_move: Dictionary = combat.apply_player_movement(state, Vector2i(0, 0))
	expect.call(not invalid_after_move.has("last_player_movement"), "An invalid movement request should not retain an older successful movement result")
	expect.call(combat.player_movement_remaining(invalid_after_move) == 1, "An invalid movement request should preserve the remaining pool")
