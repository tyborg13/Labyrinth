...
static func run(tree: SceneTree, expect: Callable) -> void:
	...
	_submit(board, state, {})
	await tree.process_frame
	var first: Dictionary = board.gaoler_animation_snapshot("enemy_1")
	var second: Dictionary = board.gaoler_animation_snapshot("enemy_2")
	expect.call(not first.is_empty() and not second.is_empty(), "Every living Gaoler gets a production cutout")
	if first.is_empty() or second.is_empty():
		board.queue_free()
		return
	expect.call((board.get("_gaoler_renderers") as Dictionary).size() == 2, "Other enemy types do not allocate Gaoler renderers")
	expect.call(first["texture_id"] != second["texture_id"], "Two Gaolers have independent live textures and motion")
	...
	for index: int in range(directions.size()):
		for clip: String in ["walk", "attack"]:
			_submit(board, state, {"gaoler_motion": {"enemy_1": {"clip": clip, "direction": directions[index], "phase": 0.42, "action": "strike"}}})
			var sample: Dictionary = board.gaoler_animation_snapshot("enemy_1")
			expect.call(sample["facing"] == facings[index] and sample["mirrored"] == mirrors[index], "Gaoler action follows the correct board direction")
			...
			expect.call(board.gaoler_animation_snapshot("enemy_2")["clip"] == "idle", "Animating one Gaoler never animates a second actor")
			_submit(board, state, {})
			var idle: Dictionary = board.gaoler_animation_snapshot("enemy_1")
	...
	for index: int in range(player_offsets.size()):
		...
		sample = board.gaoler_animation_snapshot("enemy_1")
	...
	_submit(board, state, {"gaoler_motion": {"enemy_1": {"clip": "attack", "direction": Vector2i(0, -1), "phase": 1.0}}})
	var recovered: Dictionary = board.gaoler_animation_snapshot("enemy_1")
	...
	var renderer: Node = (board.get("_gaoler_renderers") as Dictionary)["enemy_1"]
	...
	for entry: Array in [["pull","chain_reel",0.50],["ranged","manacle_pin",12.0/38.0],["melee","strike",0.42]]:
		expect.call(Cutout.action_clip({"kind":entry[0]},{"type":"chainbound_gaoler"})==entry[1],"Each existing action routes to its own anatomy clip")
		renderer.call("present", {"clip":"attack","action":entry[1],"phase":entry[2]},false)
		expect.call(renderer.call("snapshot")["clip"]==entry[1] and is_equal_approx(float(renderer.call("snapshot")["phase"]),float(entry[2])),"Authored poses preserve existing effect progress without retiming")
		renderer.call("present", {"clip":"attack","action":entry[1],"phase":1.0},false)
		expect.call(renderer.call("snapshot")["clip"]=="idle","Action completes into idle")
	_submit(board, state, {"gaoler_motion": {"enemy_1": {"clip": "walk", "direction": Vector2i(0, -1), "phase": 0.5}}, "reduced_motion": true})
	...
	expect.call(not bool(renderer.call("snapshot")["active"]), "Hidden Gaolers stop idle work")
	_submit(board, state, {})
	var unit: Dictionary = state["enemies"][0].duplicate(true)
	unit["key"] = "enemy_1"
	unit["role"] = "enemy"
	var anchor: Texture2D = board.call("_unit_hud_anchor_texture", unit)
	expect.call(str(anchor.get_meta("asset_source_path", "")) == Cutout.REST_PATH, "Gaoler HUD and shadows use the baked new silhouette")
	expect.call((board.call("_unit_idle_frames", unit) as Array).is_empty(), "Legacy Gaoler idle sheets are not loaded")
	...
	expect.call(board.call("_texture_for_unit", echo) == texture, "Destination preview shares its Gaoler's new art")
	...
	if dissolves.has("enemy_1"):
		expect.call((dissolves["enemy_1"].call("source_rect") as Rect2).is_equal_approx(board.call("_unit_texture_draw_rect", dying, board.call("_unit_center", dying))), "Dissolve geometry respects the padded Gaoler canvas")
	else:
		expect.call(false, "The actual death presentation owns a dissolve effect")
	_submit(board, dead_state, {})
	await tree.process_frame
	expect.call(board.gaoler_animation_snapshot("enemy_1").is_empty() and (board.get("_gaoler_renderers") as Dictionary).size() == 1, "Removed Gaolers release their viewport after death presentation")
	expect.call(board.gaoler_animation_snapshot("enemy_2")["texture_id"] == second["texture_id"], "Removing one actor preserves the other's renderer")
	var definition: Dictionary = GameData.enemy_def("chainbound_gaoler")
	expect.call(int(definition["max_hp"]) == 14 and int(definition["base_initiative"]) == 13 and int(definition["reward_embers"]) == 12, "Production presentation preserves Gaoler combat data")
	expect.call(not Cutout.uses_attack({"kind": "aoe"}, {"type": "chainbound_gaoler"}) and not Cutout.uses_attack({"kind": "melee"}, {"type": "crawler"}), "Routing excludes unrelated attacks and enemy types")
	board.queue_free()
	await tree.process_frame
...
static func fixture_state() -> Dictionary:
	...
	return {"grid": grid, "room_coord": Vector2i(4, 3), "terrain": [], "traps": [], "loot": [],
		...
		"enemies": [{"id": 1, "type": "chainbound_gaoler", "pos": Vector2i(3, 3), "hp": 14, "max_hp": 14},
			{"id": 2, "type": "chainbound_gaoler", "pos": Vector2i(5, 5), "hp": 14, "max_hp": 14},
	...
	for facing: String in ["front", "rear"]:
		...
		for index: int in range(25):
			...
			for name: String in layout["joints"]:
				...
				expect.call((actual.origin - neutral.origin - expected_offset).length() < 0.0001, "Idle keeps legs planted while the complete upper body bobs together: " + facing + "/" + name)
