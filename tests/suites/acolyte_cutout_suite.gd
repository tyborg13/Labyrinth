...
static func run(tree: SceneTree, expect: Callable) -> void:
	...
	_submit(board, state, {})
	await tree.process_frame
	var first: Dictionary = board.acolyte_animation_snapshot("enemy_1")
	var second: Dictionary = board.acolyte_animation_snapshot("enemy_2")
	expect.call(not first.is_empty() and not second.is_empty(), "Every living Acolyte gets a production cutout")
	if first.is_empty() or second.is_empty():
		board.queue_free()
		return
	expect.call((board.get("_acolyte_renderers") as Dictionary).size() == 2, "Other enemy types do not allocate Acolyte renderers")
	expect.call(first["texture_id"] != second["texture_id"], "Two Acolytes have independent live textures and motion")
	...
	for index: int in range(directions.size()):
		for clip: String in ["walk", "attack"]:
			_submit(board, state, {"acolyte_motion": {"enemy_1": {"clip": clip, "direction": directions[index], "phase": 0.42}}})
			var sample: Dictionary = board.acolyte_animation_snapshot("enemy_1")
			expect.call(sample["facing"] == facings[index] and sample["mirrored"] == mirrors[index], "Acolyte action follows the correct board direction")
			...
			expect.call(board.acolyte_animation_snapshot("enemy_2")["clip"] == "idle", "Animating one Acolyte never animates a second actor")
			_submit(board, state, {})
			var idle: Dictionary = board.acolyte_animation_snapshot("enemy_1")
	...
	for index: int in range(player_offsets.size()):
		...
		sample = board.acolyte_animation_snapshot("enemy_1")
	...
	_submit(board, state, {"acolyte_motion": {"enemy_1": {"clip": "attack", "direction": Vector2i(0, -1), "phase": 1.0}}})
	var recovered: Dictionary = board.acolyte_animation_snapshot("enemy_1")
	...
	var renderer: Node = (board.get("_acolyte_renderers") as Dictionary)["enemy_1"]
	...
	for boundary: Vector2 in [Vector2(0.18,0.66), Vector2(4.0/48.0,12.0/48.0)]:
		var contact: float = boundary.y
		renderer.call("present", {"clip": "attack", "phase": contact, "release": boundary.x, "contact": contact}, false)
		expect.call(is_equal_approx(float(renderer.call("snapshot")["phase"]), 0.66), "Default and elemental casts reach the authored arrival pose at their existing result boundary")
	...
	expect.call(Cutout.uses_attack({"kind":"ranged"},{"type":"acolyte"}), "The acolyte routes ranged casting")
	for boundary: Vector2 in [Vector2(0.18,0.66), Vector2(4.0/48.0,12.0/48.0)]:
		expect.call(is_equal_approx(Cutout.attack_pose_phase(boundary.x,boundary.x,boundary.y),0.18), "Palm release matches the existing projectile start")
	_submit(board, state, {"acolyte_motion": {"enemy_1": {"clip": "walk", "direction": Vector2i(0, -1), "phase": 0.5}}, "reduced_motion": true})
	...
	var rest_socket: Vector2 = renderer.call("source_socket")
	expect.call(rest_socket.is_equal_approx(renderer.call("source_socket", true)), "Reduced-motion projectiles originate at the visible still orb")
	var clipped_origin := Vector2(37, 59)
	var cast: Dictionary = {"kind": "ranged", "acolyte_cast": true, "actor_key": "enemy_1"}
	expect.call(not (board.call("_protagonist_launch_point", cast, clipped_origin) as Vector2).is_equal_approx(clipped_origin), "Visible Acolyte casts use their orb socket")
	cast["umbra_action_clipped"] = true
	for reduced: bool in [false, true]:
		_submit(board, state, {"reduced_motion": reduced, "visible_enemy_ids": [2, 3]})
		expect.call((board.call("_protagonist_launch_point", cast, clipped_origin) as Vector2).is_equal_approx(clipped_origin), "Umbra impact and reduced-motion casts preserve the visible segment origin")
	_submit(board, state, {"visible_enemy_ids": [2, 3]})
	expect.call(not bool(renderer.call("snapshot")["active"]), "Hidden Acolytes stop idle work")
	_submit(board, state, {})
	var unit: Dictionary = state["enemies"][0].duplicate(true)
	unit["key"] = "enemy_1"
	unit["role"] = "enemy"
	var anchor: Texture2D = board.call("_unit_hud_anchor_texture", unit)
	expect.call(str(anchor.get_meta("asset_source_path", "")) == Cutout.REST_PATH, "Acolyte HUD and shadows use the baked new silhouette")
	expect.call((board.call("_unit_idle_frames", unit) as Array).is_empty(), "Legacy Acolyte idle sheets are not loaded")
	...
	expect.call(board.call("_texture_for_unit", echo) == texture, "Destination preview shares its Acolyte's new art")
	...
	if dissolves.has("enemy_1"):
		expect.call((dissolves["enemy_1"].call("source_rect") as Rect2).is_equal_approx(board.call("_unit_texture_draw_rect", dying, board.call("_unit_center", dying))), "Dissolve geometry respects the padded Acolyte canvas")
	else:
		expect.call(false, "The actual death presentation owns a dissolve effect")
	_submit(board, dead_state, {})
	await tree.process_frame
	expect.call(board.acolyte_animation_snapshot("enemy_1").is_empty() and (board.get("_acolyte_renderers") as Dictionary).size() == 1, "Removed Acolytes release their viewport after death presentation")
	expect.call(board.acolyte_animation_snapshot("enemy_2")["texture_id"] == second["texture_id"], "Removing one actor preserves the other's renderer")
	var definition: Dictionary = GameData.enemy_def("acolyte")
	expect.call(int(definition["max_hp"]) == 12 and int(definition["base_initiative"]) == 12 and int(definition["reward_embers"]) == 10, "Production presentation preserves Acolyte combat data")
	expect.call(not Cutout.uses_attack({"kind": "melee"}, {"type": "acolyte"}) and not Cutout.uses_attack({"kind": "melee"}, {"type": "crawler"}), "Routing excludes unrelated attacks and enemy types")
	board.queue_free()
	await tree.process_frame
...
static func fixture_state() -> Dictionary:
	...
	return {"grid": grid, "room_coord": Vector2i(4, 3), "terrain": [], "traps": [], "loot": [],
		...
		"enemies": [{"id": 1, "type": "acolyte", "pos": Vector2i(3, 3), "hp": 12, "max_hp": 12},
			{"id": 2, "type": "acolyte", "pos": Vector2i(5, 5), "hp": 12, "max_hp": 12},
			{"id": 3, "type": "veilbound_acolyte", "pos": Vector2i(6, 6), "hp": 9, "max_hp": 9}]}
...
static func _verify_rigid_idle(renderer: Node, expect: Callable) -> void:
	for facing: String in ["front", "rear"]:
		...
		for index: int in range(25):
			...
			var chest_offset: Vector2 = _world(pose, layout, "torso").origin - _world(rest, layout, "torso").origin
			...
			for name: String in layout["joints"]:
				...
				var fixed: bool = name == "root" or name.begins_with("hem_")
...
static func _world(pose: Dictionary, layout: Dictionary, name: String) -> Transform2D:
	...
	return _world(pose, layout, Cutout.Motion.parent(layout,name)) * Transform2D(float(p["rotation"]),Vector2(p["scale"]),float(p["skew"]),Vector2(p["position"]))
