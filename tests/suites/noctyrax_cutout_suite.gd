...
static func run(tree: SceneTree, expect: Callable) -> void:
	...
	_submit(board, state, {})
	await tree.process_frame
	var first: Dictionary = board.noctyrax_animation_snapshot("enemy_1")
	var second: Dictionary = board.noctyrax_animation_snapshot("enemy_2")
	expect.call(not first.is_empty() and not second.is_empty(), "Every living Noctyrax gets a production cutout")
	if first.is_empty() or second.is_empty():
		board.queue_free()
		return
	expect.call((board.get("_noctyrax_renderers") as Dictionary).size() == 2, "Other enemy types do not allocate Noctyrax renderers")
	expect.call(first["texture_id"] != second["texture_id"], "Two Noctyraxs have independent live textures and motion")
	...
	for index: int in range(directions.size()):
		for clip: String in ["walk", "claw", "breath", "coil", "eclipse"]:
			_submit(board, state, {"noctyrax_motion": {"enemy_1": {"clip": clip, "direction": directions[index], "phase": 0.42}}})
			var sample: Dictionary = board.noctyrax_animation_snapshot("enemy_1")
			expect.call(sample["facing"] == facings[index] and sample["mirrored"] == mirrors[index], "Noctyrax action follows the correct board direction")
			...
			expect.call(board.noctyrax_animation_snapshot("enemy_2")["clip"] == "idle", "Animating one Noctyrax never animates a second actor")
			_submit(board, state, {})
			var idle: Dictionary = board.noctyrax_animation_snapshot("enemy_1")
	...
	for index: int in range(player_offsets.size()):
		...
		sample = board.noctyrax_animation_snapshot("enemy_1")
	...
	_submit(board, state, {"noctyrax_motion": {"enemy_1": {"clip": "claw", "direction": Vector2i(0, -1), "phase": 1.0}}})
	...
	for clip: String in ["claw", "coil", "eclipse"]:
		for contact: float in [0.42, 0.66, 0.38]:
			renderer.call("present", {"clip": clip, "phase": contact, "contact": contact}, false)
			expect.call(is_equal_approx(float(renderer.call("snapshot")["phase"]), 0.5), "Every action releases at the existing effect boundary")
			renderer.call("present", {"clip": clip, "phase": 1.0}, false)
			expect.call(renderer.call("snapshot")["clip"] == "idle", "Every completed action returns to cutout idle")
	expect.call(is_equal_approx(Cutout.breath_pose_phase(0.18),0.5) and is_equal_approx(Cutout.breath_pose_phase(0.66),0.63), "Breath aims at projectile launch and holds until impact")
	for effect: Dictionary in [{"kind":"melee"},{"kind":"ranged"},{"kind":"aoe"},{"kind":"aoe","action_type":"umbra_eclipse"}]:
		expect.call(Cutout.uses_attack(effect,{"type":"noctyrax"}), "All Noctyrax action families route into the cutout")
	expect.call(Cutout.clip_for_effect({"kind":"aoe","action_type":"umbra_eclipse"}) == "eclipse", "Eclipse takes precedence over radial coil")
	expect.call(Cutout.clip_for_effect({"kind":"ranged"}) == "breath" and Cutout.clip_for_effect({"kind":"melee"}) == "claw", "Maw and claw routing stay distinct")
	expect.call(board.call("_umbra_projectile_fragment_texture", {"noctyrax_breath": true}, 0.5) == null, "Clipped Starless Breath retains shadow geometry instead of leaking a physical arrow into its visible fragment")
	expect.call(board.call("_umbra_projectile_fragment_texture", {"kind": "ranged"}, 0.5) != null, "Ordinary physical projectiles retain their existing sprite")
	_submit(board, state, {"noctyrax_motion": {"enemy_1": {"clip": "walk", "direction": Vector2i(0, -1), "phase": 0.5}}, "reduced_motion": true})
	...
	expect.call(not bool(renderer.call("snapshot")["active"]), "Hidden Noctyraxs stop idle work")
	_submit(board, state, {})
	var unit: Dictionary = state["enemies"][0].duplicate(true)
	unit["key"] = "enemy_1"
	unit["role"] = "enemy"
	var anchor: Texture2D = board.call("_unit_hud_anchor_texture", unit)
	expect.call(str(anchor.get_meta("asset_source_path", "")) == Cutout.REST_PATH, "Noctyrax HUD and shadows use the baked new silhouette")
	expect.call((board.call("_unit_idle_frames", unit) as Array).is_empty(), "Legacy Noctyrax idle sheets are not loaded")
	...
	expect.call(board.call("_texture_for_unit", echo) == texture, "Destination preview shares its Noctyrax's new art")
	...
	if dissolves.has("enemy_1"):
		expect.call((dissolves["enemy_1"].call("source_rect") as Rect2).is_equal_approx(board.call("_unit_texture_draw_rect", dying, board.call("_unit_center", dying))), "Dissolve geometry respects the padded Noctyrax canvas")
	else:
		expect.call(false, "The actual death presentation owns a dissolve effect")
	_submit(board, dead_state, {})
	await tree.process_frame
	expect.call(board.noctyrax_animation_snapshot("enemy_1").is_empty() and (board.get("_noctyrax_renderers") as Dictionary).size() == 1, "Removed Noctyraxs release their viewport after death presentation")
	expect.call(board.noctyrax_animation_snapshot("enemy_2")["texture_id"] == second["texture_id"], "Removing one actor preserves the other's renderer")
	var definition: Dictionary = GameData.enemy_def("noctyrax")
	expect.call(int(definition["max_hp"]) == 72 and int(definition["base_initiative"]) == 14 and int(definition["reward_embers"]) == 120, "Production presentation preserves Noctyrax combat data")
	expect.call(not Cutout.uses_attack({"kind": "block"}, {"type": "noctyrax"}) and not Cutout.uses_attack({"kind": "melee"}, {"type": "crawler"}), "Routing excludes unrelated attacks and enemy types")
	expect.call(int(definition["footprint"][0]) == 2 and int(definition["footprint"][1]) == 2 and bool(definition["boss_bar"]), "Noctyrax retains the 2x2 boss footprint and boss bar")
	board.queue_free()
	await tree.process_frame
...
static func fixture_state() -> Dictionary:
	...
	return {"grid": grid, "room_coord": Vector2i(4, 3), "terrain": [], "traps": [], "loot": [],
		"player": {"pos": Vector2i(6, 3), "hp": 24, "max_hp": 24},
		"enemies": [{"id": 1, "type": "noctyrax", "pos": Vector2i(3, 3), "hp": 72, "max_hp": 72, "footprint": Vector2i(2,2)},
...
static func _verify_rigid_idle(renderer: Node, expect: Callable) -> void:
	for facing: String in ["front", "rear"]:
		...
		for index: int in range(25):
			...
			var body_offset: Vector2 = Cutout.Motion._world(pose, layout, "body").origin - Cutout.Motion._world(rest, layout, "body").origin
			lowest = minf(lowest, body_offset.y)
