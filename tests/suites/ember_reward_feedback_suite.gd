...
static func _test_live_header_feedback(tree: SceneTree, expect: Callable) -> void:
	...
	expect.call(gain_label != null and gain_label.text == "+8", "Ember gain should use one terse +N label")
	if gain_label != null:
	...
	await tree.process_frame
	expect.call(stats.text.ends_with("EMBERS 48"), "The ember counter should reach the exact awarded total")
