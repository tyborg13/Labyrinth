...
const Cutout = preload("res://scripts/chainbound_gaoler_cutout/renderer.gd")
const ProofInputs = preload("res://tests/chainbound_gaoler_proof_inputs.gd")
const OUTPUT: String = "user://probes/chainbound_gaoler_gameplay_v3"
...
func _initialize() -> void:
	...
	if DisplayServer.get_name() == "headless":
		push_error("Actual RunScene playback requires a native renderer for its retained-cache frame boundaries")
		quit(1)
		return
	call_deferred("_run")
...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://gaoler_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://gaoler_probe_run.save")
	...
	for index: int in range(directions.size()):
		for intent_id: String in ["chain_reel", "manacle_pin", "cudgel_press"]:
			var origin := Vector2i(3, 3)
			await _fixture(origin + directions[index] * 2, origin, false, intent_id)
			_texture_id = int(_snapshot().get("texture_id", 0))
			_assert(_texture_id != 0, "Actual RunScene owns the Gaoler texture")
			if intent_id == "chain_reel":
			...
			await _record("%02d_%s_%s" % [index + 1, intent_id, _direction_name(index)], false, true)
			var after: Dictionary = _instance.get("_combat_state")
			_assert(after["player"] == expected["player"], "Animation preserves exact player damage, displacement and status for " + intent_id)
			_assert(after["enemies"] == expected["enemies"], "Animation preserves Gaoler positions, intent and block for " + intent_id)
			_assert(int(after["initiative_clock"]) == int(expected["initiative_clock"]), "Animation preserves initiative resolution")
			_assert(int(after["player"]["hp"]) == {"chain_reel":36,"manacle_pin":39,"cudgel_press":34}[intent_id], "Existing damage applies exactly once for " + intent_id)
	...
	for intent_id: String in ["chain_reel", "manacle_pin", "cudgel_press", "iron_guard"]:
		...
		var expected: Dictionary = engine.advance_to_next_player_turn_with_steps(engine.finish_player_activation(before))["state"]
		_instance.call("_on_pass_turn_pressed")
		await _record("06_reduced_" + intent_id, false, false)
		var after: Dictionary = _instance.get("_combat_state")
		_assert(after["player"] == expected["player"] and after["enemies"] == expected["enemies"], "Reduced motion preserves all results for " + intent_id)
		_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the new Gaoler still")
	await _fixture(Vector2i(3, 5), Vector2i(3, 3), false, "iron_guard")
	_texture_id = int(_snapshot()["texture_id"])
	_instance.call("_on_pass_turn_pressed")
	await _record("07_iron_guard", false, false)
	_assert(int((_instance.get("_combat_state") as Dictionary)["enemies"][0].get("block", 0)) == 5, "Iron Guard retains five self block")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "cudgel_press", 9)
	_texture_id = int(_snapshot()["texture_id"])
	...
	await _record("09_gaoler_death", true, false)
	_assert(_snapshot().is_empty(), "Dead Gaoler releases its cutout after the dissolve")
	...
	_assert(survivors.size() == 1 and int(survivors[0]["id"]) == 2, "The other Gaoler remains alive after the exact lethal hit")
	...
	for index: int in range(around.size()):
		...
		_assert(_snapshot()["clip"] == "idle" and _snapshot()["facing"] == facing["facing"] and _snapshot()["mirrored"] == facing["mirrored"], "Gaoler idle turns toward the player's completed destination")
	...
	var portrait: String = str(_instance.TURN_ORDER_PORTRAITS.get("chainbound_gaoler", ""))
	_assert(portrait == "res://assets/art/portraits/chainbound_gaoler.png" and preload("res://scripts/asset_loader.gd").load_texture_source_first(portrait) != null, "Turn clock retains the dedicated Chainbound Gaoler portrait")
	_assert(capture_inputs == ProofInputs.capture(), "Production inputs remain unchanged during native gameplay capture")
	_manifest["capture_input_sha256"] = capture_inputs
	...
	print("GAOLER GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "cudgel_press", gaoler_hp: int = 14) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Chainbound Gaoler Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "chainbound_gaoler", "pos": enemy_tile, "hp": gaoler_hp, "max_hp": 14, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "chainbound_gaoler", "pos": Vector2i(6, 6), "hp": 14, "max_hp": 14, "block": 0})
...
func _record(label: String, allow_death: bool, require_attack: bool, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		_assert((allow_death and snapshot.is_empty()) or int(snapshot.get("texture_id", 0)) == _texture_id, "One Gaoler texture remains live through " + label)
		...
		if not observer_before.is_empty() and str(player_motion.get("clip", "")) == "walk":
			...
			_assert(snapshot["facing"] == observer_before["facing"] and snapshot["mirrored"] == observer_before["mirrored"], "Gaoler waits until player movement finishes before turning")
		...
		for candidate: Dictionary in _board.call("_visible_units"):
			if str(candidate.get("key", "")) == "enemy_1":
				unit = candidate
		if str(snapshot.get("clip", "")) in ["chain_reel", "manacle_pin", "strike"]:
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Gaoler attack keeps its planted stance on its actual tile")
			var progress: float = float(presentation.get("effect_progress", 0.0))
			var boundary: float = 0.5 if str(snapshot["clip"]) == "chain_reel" else float(_instance.call("_attack_feedback_start_progress", effect))
		...
		if str(snapshot.get("clip", "")) == "walk" and not unit.is_empty():
			...
			if previous_support.get("view", "") == support["view"]:
				for foot: String in ["foot_hook", "foot_fist"]:
	...
	if require_attack:
		_assert(phases.has("chain_reel") or phases.has("manacle_pin") or phases.has("strike"), "Actual End Turn plays the correct Gaoler attack in " + label)
		if label.contains("cudgel_press"):
			_assert(phases.has("walk") and phases.has("strike"), "Cudgel Press plays actual approach and strike")
	...
	if not images.is_empty():
		...
		for target: float in [0.32, 0.55, 0.65]:
			...
			for index: int in range(samples.size()):
				if str(samples[index]["animation"].get("clip", "")) not in ["chain_reel", "manacle_pin", "strike"]:
					continue
...
func _support_points(unit: Dictionary, snapshot: Dictionary) -> Dictionary:
	var renderer: Node = (_board.get("_gaoler_renderers") as Dictionary)["enemy_1"]
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "chain_reel", "manacle_pin", "strike"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Gaoler movement and melee face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("gaoler_animation_snapshot", "enemy_1")
