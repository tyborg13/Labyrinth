...
func _capture_feedback_surfaces() -> void:
	...
	if reward_widget != null:
		...
		create_timer(0.10).timeout.connect(_assert_sfx_entry.bind(instance, 4.863, "Reward accepted"))
	...
	create_timer(0.10).timeout.connect(_assert_sfx_entry.bind(instance, 4.863, "Relic reward accepted"))
