...
func _run() -> void:
	...
	if supports_staging:
		...
		_expect(prepared_player != null and str(prepared_player.get_meta("asset_source_path", "")).begins_with("res://assets/units/protagonist_cutout/"), "Staging must prepare the approved cutout rest texture without a live renderer")
		...
		await _present_frame()
		var cutout: Node = async_board.get("_protagonist_renderer")
		var cutout_count: int = 0
		for child: Node in async_board.get_children():
			if child.get_script() == preload("res://scripts/protagonist_cutout/renderer.gd"):
				cutout_count += 1
		_expect(is_instance_valid(cutout) and cutout.is_node_ready() and cutout_count == 1, "Staged board ready must create exactly one live cutout renderer")
		_expect((async_board.get("_unit_textures") as Dictionary).get("player") == prepared_player, "Ready must retain the staged cutout rest texture identity")
		...
		for layer: Control in async_board.call("_retained_render_layers"):
			_expect(layer.get("_protagonist_renderer") == cutout, "Retained layers must share the owner cutout renderer")
		...
		# The live renderer can still be inside a frame callback after staging.
		async_board.queue_free()
		await process_frame
	else:
		async_board.free()
