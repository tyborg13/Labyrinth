...
## Packs production scripts and the thirteen Guardian cutout asset directories. This
...
func _initialize() -> void:
	...
	_pack_directory(pack,"res://assets/units/guardians",["png","json"])
	_check(pack.add_file("res://runtime.gd","res://tests/fixtures/guardian_cutout_export_smoke.gd") == OK, "Runtime smoke script packs")
	...
	for error: String in _errors:
		push_error(error)
	print("GUARDIAN PACK BUILD: " + ("PASS" if _errors.is_empty() else "FAIL"))
