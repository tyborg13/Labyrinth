...
const ProductionRig = preload("res://scripts/zekarion_cutout/rig.gd")
const CaseRig = preload("res://tools/cutout_pipeline/rig.gd")
const OUTPUT: String = "user://probes/zekarion_cutout_assets"
...
func _run() -> void:
	...
	for facing: String in ["front", "rear"]:
		...
		_check(reference.configure("res://experiments/cutouts/zekarion/v01/cutout.json"), "Reviewed case loads")
		...
		for clip: String in ["rest","idle","walk","claw","breath","charge","call"]:
			var frames: int = 1 if clip == "rest" else 24 if clip == "idle" else 32
			for index: int in range(frames):
				var phase: float = float(index) / float(frames - 1 if clip not in ["idle","walk"] else frames)
				...
				compared += 1
				if clip not in ["rest","idle","walk"] and index in [10,17]:
					image.save_png(OUTPUT.path_join("%s_%s_%02d.png" % [facing,clip,index]))
				if clip == "rest":
					var baked: Image = Image.load_from_file("res://assets/units/zekarion_cutout/" + facing + "/rest.png")
	...
	print("ZEKARION ASSET PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
