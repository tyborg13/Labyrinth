...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://warden_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://warden_probe_run.save")
	...
	for index: int in range(directions.size()):
		...
		await _record("%02d_marching_%s" % [index + 1, _direction_name(index)], false, true)
		var after: Dictionary = _instance.get("_combat_state")
		_assert(int(after["player"]["hp"]) == 31 and int(after["player"]["hp"]) == int(expected["player"]["hp"]), "Marching Blow applies exactly nine damage once")
		_assert(after["enemies"][0]["pos"] == origin + directions[index] and after["enemies"][0]["pos"] == expected["enemies"][0]["pos"], "Marching Blow follows the resolved one-tile route")
	...
	await _fixture(Vector2i(5, 4), Vector2i(3, 3), false, "crushing_step")
	_texture_id = int(_snapshot()["texture_id"])
	_instance.call("_on_pass_turn_pressed")
	await _record("05_crushing_step", false, true)
	_assert(int((_instance.get("_combat_state") as Dictionary)["player"]["hp"]) == 34, "Crushing Step preserves its six damage area result")
	await _fixture(Vector2i(3, 5), Vector2i(3, 3), true)
	_texture_id = int(_snapshot()["texture_id"])
	_instance.call("_on_pass_turn_pressed")
	await _record("06_reduced_marching", false, false)
	_assert(int((_instance.get("_combat_state") as Dictionary)["player"]["hp"]) == 31, "Reduced motion preserves the same movement and damage")
	_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the new Warden still")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "bulwark")
	_texture_id = int(_snapshot()["texture_id"])
	_instance.call("_on_pass_turn_pressed")
	await _record("07_bulwark", false, false)
	_assert(int((_instance.get("_combat_state") as Dictionary)["enemies"][1].get("block", 0)) == 6, "Bulwark still protects the other Warden")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "marching_blow", 9)
	_texture_id = int(_snapshot()["texture_id"])
	...
	await _record("09_warden_death", true, false)
	_assert(_snapshot().is_empty(), "Dead Warden releases its cutout after the dissolve")
	...
	_assert(survivors.size() == 1 and int(survivors[0]["id"]) == 2, "The other Warden remains alive after the exact lethal hit")
	...
	print("WARDEN GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "marching_blow", warden_hp: int = 18) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Stone Warden Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "warden", "pos": enemy_tile, "hp": warden_hp, "max_hp": 18, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "warden", "pos": Vector2i(6, 6), "hp": 18, "max_hp": 18, "block": 0})
	...
	for intent: Dictionary in GameData.enemy_def("warden")["intents"]:
...
func _record(label: String, allow_death: bool, require_attack: bool, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		if str(snapshot.get("clip", "")) == "attack":
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Warden attack keeps its planted stance on its actual tile")
	...
	if require_attack:
		_assert(phases.has("walk") and phases.has("attack"), "Actual End Turn plays the Warden walk and attack in " + label)
...
func _support_points(unit: Dictionary, snapshot: Dictionary) -> Dictionary:
	var renderer: Node = (_board.get("_warden_renderers") as Dictionary)["enemy_1"]
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "attack"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Warden movement and melee face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("warden_animation_snapshot", "enemy_1")
