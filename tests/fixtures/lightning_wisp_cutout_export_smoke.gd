...
func _ready() -> void:
	var renderer_script: Script = load("res://scripts/lightning_wisp_cutout/renderer.gd") as Script
	...
	for facing: String in ["front", "rear"]:
		...
		passed = passed and (rig.get("load_errors") as PackedStringArray).is_empty() and (rig.get("bones") as Dictionary).size() == 6
		for clip: String in ["idle", "walk", "attack", "cast"]:
			rig.call("apply_pose", clip, 0.42)
	renderer.queue_free()
	await get_tree().process_frame
	print("LIGHTNING WISP CUTOUT EXPORT RUNTIME: " + ("PASS" if passed else "FAIL") + " (editor=" + str(OS.has_feature("editor")) + ")")
