...
func _run() -> void:
	...
	_assert(reused_proxy.visible and reused_proxy.top_level and reused_proxy.process_mode == Node.PROCESS_MODE_INHERIT, "Reacquired proxies should restore visible top-level effect processing")
	_assert(reused_proxy.position == source_rect.position, "Reacquired proxies should use the new effect origin")
	_assert(is_zero_approx(reused_proxy.rotation) and reused_proxy.pivot_offset == Vector2.ZERO, "Reacquired proxies should clear consumed-card rotation and pivot state")
	...
	root.add_child(pooled_a)
	root.add_child(pooled_b)
