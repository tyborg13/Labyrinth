...
const ProductionRig = preload("res://scripts/iskaldra_cutout/rig.gd")
const CaseRig = preload("res://tools/cutout_pipeline/rig.gd")
const OUTPUT: String = "user://probes/iskaldra_cutout_assets"
...
func _run() -> void:
	...
	for view: int in range(4):
		var facing: String = "front" if view < 2 else "rear"
		var mirrored: bool = view % 2 == 1
		var label: String = facing + ("_reflected" if mirrored else "")
		...
		_check(reference.configure("res://experiments/cutouts/iskaldra/v01/cutout.json"), "Reviewed case loads")
		...
		for clip: String in ["rest", "idle", "walk", "talon", "lance", "storm", "mantle"]:
			var frames: int = 1 if clip == "rest" else 24 if clip == "idle" else 40
			for index: int in range(frames):
				var phase: float = 0.0 if frames == 1 else float(index) / float(frames - 1 if clip not in ["walk","idle"] else frames)
				...
				compared += 1
				var folder: String = OUTPUT.path_join(label + "_" + clip)
				DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(folder))
				image.save_png(folder.path_join("pose_%04d.png" % index))
				if clip == "rest" and not mirrored:
					var baked: Image = Image.load_from_file("res://assets/units/iskaldra_cutout/" + facing + "/rest.png")
	...
	output.store_string(JSON.stringify({"ok": _errors.is_empty(), "native_identical_frames": compared, "directions": 4, "sampling": "All clips include both directional reflections at common 512px registration", "errors": _errors}, "\t"))
	output.close()
	actual.free()
	accepted.free()
	...
	print("ISKALDRA ASSET PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
