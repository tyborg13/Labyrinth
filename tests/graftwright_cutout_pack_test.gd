...
func _initialize() -> void:
	...
	for path: String in ["scripts/graftwright_cutout/rig.gd", "scripts/graftwright_cutout/motion.gd", "scripts/protagonist_cutout/rig.gd", "scripts/protagonist_cutout/motion.gd", "scripts/asset_loader.gd"]:
		check(pack.add_file("res://" + path, "res://" + path) == OK, "Pack " + path)
	pack_directory(pack, "res://assets/units/graftwright_cutout")
	check(pack.add_file("res://runtime.gd", "res://tests/fixtures/graftwright_cutout_export_smoke.gd") == OK, "Pack runtime test")
	...
	print("GRAFTWRIGHT PRODUCTION PACK: " + ("FAIL" if failed else "PASS"))
