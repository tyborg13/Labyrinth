...
const OUTPUT: String = "user://probes/crawler_gameplay_v1"
...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://crawler_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://crawler_probe_run.save")
	...
	for index: int in range(directions.size()):
		...
		_assert(_texture_id != 0, "Actual RunScene owns the crawler texture")
		await _record("00_idle_" + _direction_name(index), false, false, {}, 2.8)
		...
		await _record("%02d_skitter_%s" % [index + 1, _direction_name(index)], false, true)
		_verify_outcome(expected, "skitter_strike")
		_check_direction(index)
	for index: int in range(directions.size()):
		var origin := Vector2i(3, 3) if index < 2 else Vector2i(4, 4)
		await _fixture(origin + directions[index] * 3, origin, false, "lunge")
		_texture_id = int(_snapshot()["texture_id"])
		var engine := CombatEngine.new()
		var before: Dictionary = (_instance.get("_combat_state") as Dictionary).duplicate(true)
		var expected: Dictionary = engine.advance_to_next_player_turn_with_steps(engine.finish_player_activation(before))["state"]
		_instance.call("_on_pass_turn_pressed")
		await _record("%02d_lunge_%s" % [index + 5, _direction_name(index)], false, true)
		_verify_outcome(expected, "lunge")
		var variants: Array = []
		for sample: Dictionary in _manifest["clips"][-1]["samples"]:
			if str(sample["animation"].get("clip", "")) == "attack":
				variants.append(str(sample["animation"].get("variant", "")))
		_assert(not variants.is_empty() and variants.all(func(value: Variant) -> bool: return value == "lunge"), "Real Lunge uses its distinct body-committed claw animation")
		_check_direction(index)
	await _fixture(Vector2i(3, 5), Vector2i(3, 3), true)
	_texture_id = int(_snapshot()["texture_id"])
	var engine := CombatEngine.new()
	var expected: Dictionary = engine.advance_to_next_player_turn_with_steps(engine.finish_player_activation((_instance.get("_combat_state") as Dictionary).duplicate(true)))["state"]
	_instance.call("_on_pass_turn_pressed")
	await _record("09_reduced_skitter", false, false)
	_verify_outcome(expected, "reduced_skitter")
	_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the same crawler still")
	await _fixture(Vector2i(3, 6), Vector2i(3, 3), false, "coil")
	_texture_id = int(_snapshot()["texture_id"])
	expected = engine.advance_to_next_player_turn_with_steps(engine.finish_player_activation((_instance.get("_combat_state") as Dictionary).duplicate(true)))["state"]
	_instance.call("_on_pass_turn_pressed")
	await _record("10_coil", false, false)
	_verify_outcome(expected, "coil")
	_assert((_manifest["clips"][-1]["phases_seen"] as Array).has("coil"), "Coil plays its restrained defensive curl")
	_assert(int((_instance.get("_combat_state") as Dictionary)["enemies"][0].get("block", 0)) == 4, "Coil grants exactly four block to its own actor")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "skitter_strike", 9)
	_texture_id = int(_snapshot()["texture_id"])
	...
	await _record("12_crawler_death", true, false)
	_assert(_snapshot().is_empty(), "Dead Crawler releases its cutout after the dissolve")
	...
	_assert(survivors.size() == 1 and int(survivors[0]["id"]) == 2, "The other Crawler remains alive after the exact lethal hit")
	...
	for index: int in range(around.size()):
		...
		await _record("%02d_player_reposition_%s" % [15 + index, _direction_name((index + 1) % 4)], false, false, before)
		...
		_assert(_snapshot()["clip"] == "idle" and _snapshot()["facing"] == facing["facing"] and _snapshot()["mirrored"] == facing["mirrored"], "Crawler idle turns toward the player's completed destination")
	...
	var portrait: String = str(_instance.TURN_ORDER_PORTRAITS.get("crawler", ""))
	_assert(portrait == "res://assets/art/portraits/tunnel_crawler.png" and preload("res://scripts/asset_loader.gd").load_texture_source_first(portrait) != null, "Turn clock retains the dedicated Tunnel Crawler portrait")
	...
	print("CRAWLER GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _verify_outcome(expected: Dictionary, intent: String) -> void:
	var after: Dictionary = _instance.get("_combat_state")
	var player: Dictionary = after["player"]
	var expected_player: Dictionary = expected["player"]
	_assert(int(player["hp"]) == int(expected_player["hp"]) and int(player.get("bleed", 0)) == int(expected_player.get("bleed", 0)), "Claw damage and bleed exactly match the unchanged resolver: " + intent)
	_assert(after["enemies"][0]["pos"] == expected["enemies"][0]["pos"] and int(after["enemies"][0].get("block", 0)) == int(expected["enemies"][0].get("block", 0)), "Resolved travel and block remain unchanged: " + intent)
	_assert(int(after["initiative_clock"]) == int(expected["initiative_clock"]), "Initiative is preserved: " + intent)
	_manifest["outcomes"].append({"intent":intent, "hp":player["hp"], "bleed":player.get("bleed", 0), "enemy_pos":after["enemies"][0]["pos"], "block":after["enemies"][0].get("block", 0), "initiative_clock":after["initiative_clock"], "resolver_equal":player["hp"] == expected_player["hp"] and after["enemies"][0]["pos"] == expected["enemies"][0]["pos"]})

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "skitter_strike", crawler_hp: int = 14) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Tunnel Crawler Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "crawler", "pos": enemy_tile, "hp": crawler_hp, "max_hp": 14, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "crawler", "pos": Vector2i(6, 6), "hp": 14, "max_hp": 14, "block": 0})
...
func _record(label: String, allow_death: bool, require_attack: bool, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		_assert((allow_death and snapshot.is_empty()) or int(snapshot.get("texture_id", 0)) == _texture_id, "One Crawler texture remains live through " + label)
		...
		if not observer_before.is_empty() and str(player_motion.get("clip", "")) == "walk":
			...
			_assert(snapshot["facing"] == observer_before["facing"] and snapshot["mirrored"] == observer_before["mirrored"], "Crawler waits until player movement finishes before turning")
		...
		if str(snapshot.get("clip", "")) == "attack":
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Crawler attack keeps its planted stance on its actual tile")
			...
			var expected_hp: int = 40 if progress < boundary else 40 - int(effect.get("hp_loss", 0))
			_assert(int(display_state["player"]["hp"]) == expected_hp, "Visible claw damage changes at the existing contact boundary")
	...
	if require_attack:
		_assert(phases.has("walk") and phases.has("attack"), "Actual End Turn plays the Crawler walk and attack in " + label)
...
func _support_points(unit: Dictionary, snapshot: Dictionary) -> Dictionary:
	var renderer: Node = (_board.get("_crawler_renderers") as Dictionary)["enemy_1"]
	...
	for foot: String in ["claw_near", "claw_far", "foot_near", "foot_far"]:
		...
		if not bool(state["contact"]):
			continue
		var contact_point: Array = rig.layout["landmarks"]["contacts"][foot]
		var offset: Vector2 = Vector2(float(contact_point[0]), float(contact_point[1])) - Cutout.Motion._point(rig.layout, foot)
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "attack"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Crawler movement and melee face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("crawler_animation_snapshot", "enemy_1")
