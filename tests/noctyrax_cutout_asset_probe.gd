...
const ProductionRig = preload("res://scripts/noctyrax_cutout/rig.gd")
const CaseRig = preload("res://tools/cutout_pipeline/rig.gd")
const OUTPUT: String = "user://probes/noctyrax_cutout_assets"
...
func _run() -> void:
	...
	for facing: String in ["front", "rear"]:
		...
		_check(reference.configure("res://experiments/cutouts/noctyrax/v01/cutout.json"), "Reviewed case loads")
		...
		for clip: String in ["rest", "idle", "walk", "claw", "breath", "coil", "eclipse"]:
			var frames: int = 1 if clip == "rest" else 24 if clip == "idle" else 32
			for index: int in range(frames):
				var phase: float = float(index) / float(frames - 1 if clip in ["claw","breath","coil","eclipse"] else frames)
				...
				compared += 1
				if clip in ["claw","breath","coil","eclipse"] and index in [10, 13, 17, 21]:
					image.save_png(OUTPUT.path_join("%s_%s_%02d.png" % [facing, clip, index]))
				if clip == "rest":
					var baked: Image = Image.load_from_file("res://assets/units/noctyrax_cutout/" + facing + "/rest.png")
	...
	print("NOCTYRAX ASSET PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
