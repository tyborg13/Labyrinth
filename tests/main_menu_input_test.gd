...
func _test_hover_and_press_same_frame_activates_once() -> void:
	...
	press_event.position = settings_center
	press_event.global_position = settings_center
	instance.get_viewport().push_input(press_event, true)
	await process_frame
	var release_event := InputEventMouseButton.new()
	release_event.button_index = MOUSE_BUTTON_LEFT
	release_event.pressed = false
	release_event.position = settings_center
	release_event.global_position = settings_center
