...
const OUTPUT: String = "user://probes/veilbound_acolyte_gameplay_v1"
...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://veilbound_acolyte_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://veilbound_acolyte_probe_run.save")
	...
	for index: int in range(directions.size()):
		...
		_assert(_texture_id != 0, "Actual RunScene owns the Veilbound Acolyte texture")
		await _record("idle_" + _direction_name(index), false, "", {}, 3.6)
		for intent_id: String in ["veil_needle", "closing_shade", "shroudstep"]:
			var distance: int = 1 if intent_id == "shroudstep" else 2
			await _fixture(origin + directions[index] * distance, origin, false, intent_id)
			...
			var required: String = "cast" if intent_id == "veil_needle" else "walk_attack" if intent_id == "closing_shade" else "walk"
			await _record(intent_id + "_" + _direction_name(index), false, required)
			var after: Dictionary = _instance.get("_combat_state")
			_assert(int(after["player"]["hp"]) == int(expected["player"]["hp"]), "Presentation preserves exact engine damage for " + intent_id)
			_assert(int(after["player"]["hp"]) == (40 if intent_id == "shroudstep" else 39), "Attack damage applies exactly once; retreat deals no damage")
			_assert(after["enemies"][0]["pos"] == expected["enemies"][0]["pos"], "Animation follows the resolved " + intent_id + " route")
			_assert(int(after["initiative_clock"]) == int(expected["initiative_clock"]), "Animation preserves initiative resolution")
			_manifest["clips"][-1]["outcome"] = {"intent":intent_id,"actual_hp":after["player"]["hp"],"expected_hp":expected["player"]["hp"],"actual_pos":after["enemies"][0]["pos"],"expected_pos":expected["enemies"][0]["pos"],"initiative":after["initiative_clock"]}
			if intent_id != "shroudstep":
				_check_direction(index)
	for intent_id: String in ["veil_needle", "closing_shade", "shroudstep"]:
		await _fixture(Vector2i(3,5),Vector2i(3,3),true,intent_id)
		_texture_id = int(_snapshot()["texture_id"])
		var before: Dictionary = (_instance.get("_combat_state") as Dictionary).duplicate(true)
		var expected: Dictionary = CombatEngine.new().advance_to_next_player_turn_with_steps(CombatEngine.new().finish_player_activation(before))["state"]
		_instance.call("_on_pass_turn_pressed")
		await _record("reduced_"+intent_id,false,"")
		_assert(int((_instance.get("_combat_state") as Dictionary)["player"]["hp"]) == int(expected["player"]["hp"]), "Reduced motion preserves " + intent_id + " results")
		_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the same directional cutout still")
	await _fixture(Vector2i(3,4),Vector2i(3,3),false,"closing_shade",4)
	_texture_id = int(_snapshot()["texture_id"])
	var rewards_before: int = int((_instance.get("_combat_state") as Dictionary).get("room_embers",0))
	...
	await _record("veilbound_death",true,"")
	_assert(_snapshot().is_empty(), "Defeated acolyte releases its renderer after the dissolve")
	...
	_assert(survivors.size()==1 and int(survivors[0]["id"])==2, "The other minion survives independently")
	_assert(int((_instance.get("_combat_state") as Dictionary).get("room_embers",0))==rewards_before, "Veilbound Acolyte death preserves its zero ember reward")
	...
	for index: int in range(around.size()):
		...
		_assert(_snapshot()["clip"] == "idle" and _snapshot()["facing"] == facing["facing"] and _snapshot()["mirrored"] == facing["mirrored"], "Veilbound Acolyte idle turns toward the player's completed destination")
	...
	var portrait: String = str(_instance.TURN_ORDER_PORTRAITS.get("veilbound_acolyte", ""))
	_assert(portrait == "res://assets/art/portraits/veilbound_acolyte.png" and preload("res://scripts/asset_loader.gd").load_texture_source_first(portrait) != null, "Turn clock retains the dedicated Veilbound Acolyte portrait")
	...
	print("VEILBOUND ACOLYTE GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "closing_shade", veilbound_acolyte_hp: int = 4) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Veilbound Acolyte Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "veilbound_acolyte", "pos": enemy_tile, "hp": veilbound_acolyte_hp, "max_hp": 4, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "veilbound_acolyte", "pos": Vector2i(6, 6), "hp": 4, "max_hp": 4, "block": 0})
	...
	for intent: Dictionary in GameData.enemy_def("veilbound_acolyte")["intents"]:
...
func _record(label: String, allow_death: bool, required: String, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		_assert((allow_death and snapshot.is_empty()) or int(snapshot.get("texture_id", 0)) == _texture_id, "One Veilbound Acolyte texture remains live through " + label)
		...
		if not observer_before.is_empty() and str(player_motion.get("clip", "")) == "walk":
			...
			_assert(snapshot["facing"] == observer_before["facing"] and snapshot["mirrored"] == observer_before["mirrored"], "Veilbound Acolyte waits until player movement finishes before turning")
		phases[str(snapshot.get("clip", "gone"))] = true
		var companion: Dictionary = _board.call("veilbound_acolyte_animation_snapshot","enemy_2")
		_assert(not companion.is_empty() and companion.get("clip","") in ["idle","rest"], "The second minion stays independent during " + label)
		var actor_motion: Dictionary = presentation.get("veilbound_acolyte_motion",{}).get("enemy_1",{})
		if not actor_motion.is_empty() and actor_motion.get("direction",Vector2i.ZERO)!=Vector2i.ZERO:
			var wanted: Dictionary = Cutout.direction_for_delta(actor_motion["direction"])
			_assert(snapshot["facing"]==wanted["facing"] and snapshot["mirrored"]==wanted["mirrored"], "Actions follow the actual resolved route/target direction")
		...
		for candidate: Dictionary in _board.call("_visible_units"):
			if str(candidate.get("key", "")) == "enemy_1":
				unit = candidate
		if str(snapshot.get("clip", "")) in ["attack", "cast"]:
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Veilbound Acolyte attack keeps its planted stance on its actual tile")
			...
			var is_cast: bool = snapshot["clip"]=="cast"
			var release_phase: float = 0.18 if is_cast else 0.42
			if progress >= release_phase and progress <= (0.30 if is_cast else 0.50):
				var renderer: Node = (_board.get("_veilbound_acolyte_renderers") as Dictionary)["enemy_1"]
				var rig: Node2D = renderer.get("rigs")[snapshot["facing"]]
				var bone: Node2D = rig.get("bones")["orb" if is_cast else "strike_hand"]
				var body: Rect2 = _board.call("_unit_draw_rect",unit)
				var live_socket: Vector2 = body.position + (bone.global_position-Cutout.SOURCE_OFFSET)*body.size/Cutout.SOURCE_SIZE
				var launch: Vector2 = _board.call("_veilbound_acolyte_launch_point",effect,Vector2.ZERO,is_cast)
				_assert(launch.distance_to(live_socket)<0.1,"Actual shadow feedback starts at the aimed hand/orb socket")
		...
		if str(snapshot.get("clip", "")) == "walk" and not unit.is_empty():
			...
			if previous_support.get("view", "") == support["view"]:
				for foot: String in ["support_r", "support_l"]:
		...
		if now >= next_capture:
			...
			samples.append({"seconds": float(now - started) / 1000000.0, "animation": snapshot,
				"player_motion": player_motion.duplicate(true), "actor_motion": presentation.get("veilbound_acolyte_motion",{}).duplicate(true), "effect": effect.duplicate(true), "effect_progress": float(presentation.get("effect_progress", 0.0)),
	...
	if not required.is_empty():
		for family: String in required.split("_"):
			_assert(phases.has(family), "Actual Pass plays " + family + " in " + label)
	...
	if not images.is_empty():
		...
		for target: float in [0.11, 0.18, 0.28, 0.42, 0.66, 0.85]:
			var nearest: int = -1
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "attack", "cast"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Veilbound Acolyte movement and melee face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("veilbound_acolyte_animation_snapshot", "enemy_1")
