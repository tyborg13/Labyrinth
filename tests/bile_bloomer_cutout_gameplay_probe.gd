...
const OUTPUT: String = "user://probes/bile_bloomer_gameplay_v1"
...
func _run() -> void:
	...
	ProgressionStore.set_storage_path("user://bile_bloomer_probe_progression.json")
	ProgressionStore.set_run_storage_path("user://bile_bloomer_probe_run.save")
	...
	for index: int in range(directions.size()):
		var origin: Vector2i = Vector2i(3, 3) if index < 2 else Vector2i(4, 4)
		await _fixture(origin + directions[index] * 3, origin)
		_texture_id = int(_snapshot().get("texture_id", 0))
		_assert(_texture_id != 0, "Actual RunScene owns the Bloomer texture")
		await _record("00_idle_" + _direction_name(index), false, "", {}, 3.6)
		await _perform_intent("%02d_burst_%s" % [index + 1, _direction_name(index)], "burst")
		_check_direction(index)
	var ranged_origins: Array[Vector2i]
	ranged_origins.assign([Vector2i(3, 1), Vector2i(1, 3), Vector2i(3, 6), Vector2i(6, 3)])
	for index: int in range(directions.size()):
		await _fixture(ranged_origins[index] + directions[index] * 5, ranged_origins[index], false, "spore_mark")
		_texture_id = int(_snapshot()["texture_id"])
		await _perform_intent("%02d_mark_%s" % [index + 5, _direction_name(index)], "mark")
		_check_direction(index)
	for intent: String in ["bile_burst", "spore_mark"]:
		await _fixture(Vector2i(3, 6), Vector2i(3, 3), true, intent)
		_texture_id = int(_snapshot()["texture_id"])
		await _perform_intent("09_reduced_" + intent, "")
		_assert(_snapshot()["clip"] == "rest", "Reduced motion keeps the new Bloomer still")
	for fueled: bool in [false, true]:
		await _fixture(Vector2i(3, 5), Vector2i(3, 3), false, "pustule_shell", 14, fueled)
		_texture_id = int(_snapshot()["texture_id"])
		await _perform_intent("10_shell_" + str(fueled), "")
		_assert(int((_instance.get("_combat_state") as Dictionary)["enemies"][0].get("stoneskin", 0)) == (4 if fueled else 2), "Shale Shell retains Stoneskin and optional rubble fuel")
	await _fixture(Vector2i(3, 4), Vector2i(3, 3), false, "bile_burst", 9)
	_texture_id = int(_snapshot()["texture_id"])
	await _instance.call("_on_card_pressed", 0)
	_instance.call("_on_board_tile_hovered", Vector2i(3, 3))
	await _settle()
	await _still("11_target_preview")
	_instance.call("_on_board_tile_clicked", Vector2i(3, 3))
	await _record("12_bloomer_death", true, "")
	_assert(_snapshot().is_empty(), "Dead Bloomer releases its cutout after the dissolve")
	...
	_assert(survivors.size() == 1 and int(survivors[0]["id"]) == 2, "The other Bloomer remains alive after the exact lethal hit")
	...
	if router != null:
		...
		await _instance.call("_on_card_pressed", 0)
		await _settle()
		await _still("13_controller_target")
		...
		_instance.call("_refresh_controller_interface")
		await _settle()
		await _still("14_pointer_handoff")
	...
	for index: int in range(around.size()):
		...
		await _record("%02d_player_reposition_%s" % [15 + index, _direction_name((index + 1) % 4)], false, "", before)
		...
		_assert(_snapshot()["clip"] == "idle" and _snapshot()["facing"] == facing["facing"] and _snapshot()["mirrored"] == facing["mirrored"], "Bloomer idle turns toward the player's completed destination")
	...
	var portrait: String = str(_instance.TURN_ORDER_PORTRAITS.get("bile_bloomer", ""))
	_assert(portrait == "res://assets/art/portraits/bile_bloomer.png" and preload("res://scripts/asset_loader.gd").load_texture_source_first(portrait) != null, "Turn clock retains the dedicated Shale Bloomer portrait")
	...
	print("BILE BLOOMER GAMEPLAY PROBE: " + ("PASS" if _errors.is_empty() else "FAIL"))
	quit(0 if _errors.is_empty() else 1)

func _perform_intent(label: String, action: String) -> void:
	var before: Dictionary = (_instance.get("_combat_state") as Dictionary).duplicate(true)
	...
	var after: Dictionary = _instance.get("_combat_state")
	_assert(after["player"]["hp"] == expected["player"]["hp"], "Actual action applies damage exactly once: " + label)
	_assert(after["player"].get("expose", 0) == expected["player"].get("expose", 0), "Actual action preserves Expose: " + label)
	_assert(after.get("surfaces", {}) == expected.get("surfaces", {}), "Actual action preserves every rubble tile and fuel consequence: " + label)
	_assert(after["enemies"][0]["pos"] == expected["enemies"][0]["pos"] and after["initiative_clock"] == expected["initiative_clock"], "Presentation follows the resolved path and initiative: " + label)
	_assert(after["enemies"][0].get("stoneskin", 0) == expected["enemies"][0].get("stoneskin", 0), "Actual action preserves Stoneskin: " + label)
	if not action.is_empty():
		_assert(int(after["player"]["hp"]) == 37, "Bloom attacks retain three damage")
		_assert(preload("res://scripts/path_utils.gd").manhattan(before["enemies"][0]["pos"], after["enemies"][0]["pos"]) == 1, "The fixture exercises the real one-tile root move")
		_assert(int(after["player"].get("expose", 0)) == (2 if action == "mark" else 0), "Only Shard Mark applies two Expose")
	_manifest["outcomes"].append({"label": label, "before_hp": before["player"]["hp"], "actual_hp": after["player"]["hp"], "expected_hp": expected["player"]["hp"], "expose": after["player"].get("expose", 0), "stoneskin": after["enemies"][0].get("stoneskin", 0), "surfaces": after.get("surfaces", {}), "enemy_position": after["enemies"][0]["pos"], "initiative_clock": after["initiative_clock"]})

func _fixture(player_tile: Vector2i, enemy_tile: Vector2i, reduced: bool = false, intent_id: String = "bile_burst", bloomer_hp: int = 14, fueled: bool = false) -> void:
	...
	for y: int in range(8):
		...
		grid.append(row)
	var layout: Dictionary = {"name": "Shale Bloomer Combat Trial", "coord": Vector2i(4, 3), "type": "combat", "grid": grid,
		"player_start": player_tile, "enemies": [{"id": 1, "type": "bile_bloomer", "pos": enemy_tile, "hp": bloomer_hp, "max_hp": 14, "block": 0}],
		"traps": [], "terrain": [], "element": "none"}
	layout["enemies"].append({"id": 2, "type": "bile_bloomer", "pos": Vector2i(6, 6), "hp": 14, "max_hp": 14, "block": 0})
	...
	for intent: Dictionary in GameData.enemy_def("bile_bloomer")["intents"]:
	...
	state["terrain"] = []
	if fueled:
		state = preload("res://scripts/board_surface_rules.gd").place(state, enemy_tile + Vector2i(1, 0), "rubble")
	# Revealed intents bind their fuel tile before activation. Hand-selecting
	# an intent for the probe must preserve that normal preparation step.
...
func _record(label: String, allow_death: bool, required_action: String, observer_before: Dictionary = {}, minimum_seconds: float = 0.0) -> void:
	...
	while Time.get_ticks_usec() - started < 12000000:
		...
		_assert((allow_death and snapshot.is_empty()) or int(snapshot.get("texture_id", 0)) == _texture_id, "One Bloomer texture remains live through " + label)
		_assert(_board.call("bile_bloomer_animation_snapshot", "enemy_2").get("clip", "") in ["idle", "rest"], "The other Bloomer remains independent during " + label)
		...
		if not observer_before.is_empty() and str(player_motion.get("clip", "")) == "walk":
			...
			_assert(snapshot["facing"] == observer_before["facing"] and snapshot["mirrored"] == observer_before["mirrored"], "Bloomer waits until player movement finishes before turning")
		...
		for candidate: Dictionary in _board.call("_visible_units"):
			if str(candidate.get("key", "")) == "enemy_1":
				unit = candidate
		if str(snapshot.get("clip", "")) in ["burst", "mark"] and not effect.is_empty():
			_assert((_board.call("_unit_center", unit) as Vector2).is_equal_approx(_board.call("world_position_for_tile", unit["pos"])), "Bloomer attack keeps its planted stance on its actual tile")
	...
	if not required_action.is_empty():
		_assert(phases.has("walk") and phases.has(required_action), "Actual End Turn plays the Bloomer root travel and correct attack family in " + label)
	...
	if not images.is_empty():
		...
		for target: float in [0.32, 0.55, 0.65]:
			...
			for index: int in range(samples.size()):
				if str(samples[index]["animation"].get("clip", "")) not in ["burst", "mark"]:
					continue
...
func _support_points(unit: Dictionary, snapshot: Dictionary) -> Dictionary:
	var renderer: Node = (_board.get("_bile_bloomer_renderers") as Dictionary)["enemy_1"]
...
func _check_direction(index: int) -> void:
	...
	for sample: Dictionary in _manifest["clips"][-1]["samples"]:
		...
		if str(animation.get("clip", "")) in ["walk", "burst", "mark"]:
			_assert(animation["facing"] == facings[index] and animation["mirrored"] == mirrors[index], "Bloomer movement and release face " + _direction_name(index))
	_assert(_snapshot()["facing"] == facings[index] and _snapshot()["mirrored"] == mirrors[index], "Idle retains the completed direction")

func _snapshot() -> Dictionary:
	return _board.call("bile_bloomer_animation_snapshot", "enemy_1")
