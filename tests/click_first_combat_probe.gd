...
func _capture_single_state(viewport_size: Vector2i, proof_state: String) -> void:
	...
	match proof_state:
		...
		"drag_play_state":
			...
			await _settle_ui()
			var drag_position: Vector2 = (instance.get("board_view") as Control).get_global_rect().get_center()
...
func _capture_resolution(viewport_size: Vector2i) -> void:
	...
	_assert(int(instance.get("_selected_card_index")) == 0 and str(instance.get("_selected_card_label_override")).is_empty(), "Board entry should convert Drag Full Card into printed targeting without a fallback label")
