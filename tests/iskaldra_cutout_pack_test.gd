...
func _initialize() -> void:
	...
	if args[0] == "build":
		...
		for file: String in ["scripts/enemy_cutout_facing.gd", "scripts/iskaldra_cutout/rig.gd", "scripts/iskaldra_cutout/motion.gd", "scripts/iskaldra_cutout/renderer.gd", "scripts/asset_loader.gd", "scripts/protagonist_cutout/rig.gd", "scripts/protagonist_cutout/motion.gd"]:
			_check(pack.add_file("res://" + file, "res://" + file) == OK, "Packs " + file)
		_pack_directory(pack, "res://assets/units/iskaldra_cutout")
		...
		_check(pack.add_file("res://runtime.gd", "res://tests/fixtures/iskaldra_cutout_export_smoke.gd") == OK, "Packs export-runtime entry script")
		...
		_check(not FileAccess.file_exists("res://assets/units/iskaldra_cutout/front.json"), "No loose cutout source tree is available")
		_check(ProjectSettings.load_resource_pack(args[1]), "Production cutout PCK mounts")
		var script: Script = load("res://scripts/iskaldra_cutout/renderer.gd") as Script
		...
		if script != null:
			...
			for view: String in ["front", "rear"]:
				...
				for clip: String in ["idle", "walk", "talon", "lance", "storm", "mantle"]:
					rig.call("apply_pose", clip, 0.42)
				_check((rig.get("bones") as Dictionary).size() == 17, view + " packed skeleton is complete")
			renderer.queue_free()
			await process_frame
	for message: String in _errors:
		push_error(message)
	print("ISKALDRA CUTOUT PACK TEST RESULT: " + ("PASS" if _errors.is_empty() else "FAIL") + " (" + args[0] + ", editor=" + str(OS.has_feature("editor")) + ")")
