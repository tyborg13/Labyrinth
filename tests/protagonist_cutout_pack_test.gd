...
func _initialize() -> void:
	...
	if args[0] == "build":
		...
		_check(pack.add_file("res://runtime.gd", "res://tests/fixtures/protagonist_cutout_export_smoke.gd") == OK, "Packs export-runtime entry script")
		...
		_check(not FileAccess.file_exists("res://assets/units/protagonist_cutout/front.json"), "No loose cutout source tree is available")
		_check(ProjectSettings.load_resource_pack(args[1]), "Production cutout PCK mounts")
		var script: Script = load("res://scripts/protagonist_cutout/renderer.gd") as Script
	...
	for message: String in _errors:
		push_error(message)
	print("PROTAGONIST CUTOUT PACK TEST RESULT: " + ("PASS" if _errors.is_empty() else "FAIL") + " (" + args[0] + ", editor=" + str(OS.has_feature("editor")) + ")")
