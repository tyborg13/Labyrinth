...
func _initialize() -> void:
	...
	if args[0] == "build":
		...
		for file: String in ["scripts/enemy_cutout_facing.gd", "scripts/bile_bloomer_cutout/rig.gd", "scripts/bile_bloomer_cutout/motion.gd", "scripts/bile_bloomer_cutout/renderer.gd", "scripts/asset_loader.gd", "scripts/protagonist_cutout/rig.gd", "scripts/protagonist_cutout/motion.gd"]:
			_check(pack.add_file("res://" + file, "res://" + file) == OK, "Packs " + file)
		_pack_directory(pack, "res://assets/units/bile_bloomer_cutout")
		...
		_check(pack.add_file("res://runtime.gd", "res://tests/fixtures/bile_bloomer_cutout_export_smoke.gd") == OK, "Packs export-runtime entry script")
		...
		_check(not FileAccess.file_exists("res://assets/units/bile_bloomer_cutout/front.json"), "No loose cutout source tree is available")
		_check(ProjectSettings.load_resource_pack(args[1]), "Production cutout PCK mounts")
		var script: Script = load("res://scripts/bile_bloomer_cutout/renderer.gd") as Script
		...
		if script != null:
			...
			for view: String in ["front", "rear"]:
				...
				for clip: String in ["idle", "walk", "burst", "mark"]:
	...
	for message: String in _errors:
		push_error(message)
	print("BILE BLOOMER CUTOUT PACK TEST RESULT: " + ("PASS" if _errors.is_empty() else "FAIL") + " (" + args[0] + ", editor=" + str(OS.has_feature("editor")) + ")")
