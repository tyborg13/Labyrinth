...
static func sample_pose(clip: String, phase: float, layout: Dictionary, facing: String) -> Dictionary:
	...
	if clip == "walk":
		...
		if clip == "cast":
			prepared_hand = Vector2(163,49) if rear else Vector2(106,60)
			prepared_elbow = Vector2(167,82) if rear else Vector2(111,87)
...
static func walk_foot_state(phase: float, foot: String, layout: Dictionary, facing: String) -> Dictionary:
	...
	var contact_point: Vector2 = center+lane+Vector2(walk_cycle_info(layout,facing)["direction"])*distance
