...
static func sample_pose(clip: String, phase: float, layout: Dictionary, facing: String) -> Dictionary:
	...
	if clip == "idle":
		...
		pose["upper_r"]["rotation"] = direction * (0.60 * prep + 0.45 * hit)
		pose["fore_r"]["rotation"] = direction * (0.82 * prep - 0.20 * hit)
		pose["weapon_r"]["rotation"] = direction * (0.28 * prep + (1.0 if rear else -0.45) * hit)
