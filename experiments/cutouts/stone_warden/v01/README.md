...
`recipes/registration.json` records the whole-image crops, native fitting and joint landmarks. `source/*_ownership.json` contains the reviewed semantic split polygons and explicit low-alpha edge assignments; `segmented/*/ownership.png` visualizes them. Every visible original source pixel is assigned, and both reconstructed images match their registered masters byte-for-byte. Concealed material is an additional layer, so assembled rest is not claimed to be pixel-identical to the production front.
