...
Choose fresh output directories. Omit the explicit Metal backend on other supported hosts. The evaluator's original regression script/report preserves its original temporary paths; its actual sampler change is also retained as a patch, and the complete resulting portable case is `salute_case`.
