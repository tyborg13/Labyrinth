...
static func sample_pose(clip: String, phase: float, layout: Dictionary, facing: String) -> Dictionary:
	...
	match clip:
		...
		"cast", "shoot":
			...
			if direction < 0.0:
				# The rear offhand is foreshortened beneath the cloak at rest.
				# Extension reveals the same arm length as its painted near mate;
				# the segment solver preserves sleeve width and the rigid glove.
