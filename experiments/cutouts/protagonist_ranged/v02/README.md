# Protagonist offhand actions — v02

Seeded with `seed-protagonist` from local master `6640106ac621` (approved pass-nine paint, gait, idle, and sword motion). Forked from v01 after the user requested a crossbow about three times larger. The user then requested the midpoint size; v03 supersedes this case for production. The earlier version and its proof remain intact.
...
All 21 existing joints and original paint remain. `weapon_l` is one additional rigid attachment, registered in the offhand glove. The 126×81 front and 126×96 rear crossbow drawings have independently painted viewpoints, muted walnut/leather/steel, and the same dark pixel contours as the protagonist. Grip and muzzle landmarks are in each layout's `offhand_attachment`; original generator outputs, exact prompts, and the deterministic registration recipe are under `source/`. There is no live inventory or balance change.
...
```sh
python3 tools/cutout_workflow.py validate experiments/cutouts/protagonist_ranged/v02
python3 tools/cutout_workflow.py render experiments/cutouts/protagonist_ranged/v02 --output /private/tmp/protagonist-ranged-case-v02-final --task-id protagonist-spellcast-and-offhand-crossbow-animations
python3 tools/cutout_workflow.py verify-render experiments/cutouts/protagonist_ranged/v02 --output /private/tmp/protagonist-ranged-case-v02-final
python3 tools/cutout_workflow.py inspect experiments/cutouts/protagonist_ranged/v02 --task-id protagonist-spellcast-and-offhand-crossbow-animations
```

The crossbow dimensions are exactly three times those in v01. Both were resampled from the retained original generator output, not from the tiny prior sprite. The source-space grip stays fixed; native grip and muzzle coordinates scale with the paint, preserving attachment and release alignment.
