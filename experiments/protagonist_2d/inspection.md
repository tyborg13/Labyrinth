...
This scene evaluates the Reaver with newly generated complete limbs on a live Godot skeletal rig inside the existing `CombatBoardView`. The fifth pass supplies new sleeves, hands, trousers, boots, pelvis overlap and a separate sword grip while retaining the recognizable head/body/cloak/blade artwork. Its two editable 21-bone rigs demonstrate five actions and fast walking with matched board travel. This remains a standalone inspection preview.
...
Required methods are `load_rig()->bool`, `has_facing(name)->bool`, `set_facing(name)`, `set_clip(name)`, `seek_frame(index)`, `get_frame_count()->int`, `get_fps()->float`, and `get_anchor()->Vector2` normalized to 512px. `get_reference_texture(facing)` supplies the original 255px identity image; `get_rest_texture(facing)` supplies the distinct assembled neutral reference for reconstruction checks. Optional `get_frame_index()` strengthens verification; `set_debug_bones(bool)` enables the detail-only overlay.
