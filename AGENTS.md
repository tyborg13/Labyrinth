...
- App-created worktrees can start detached and worker sandboxes often cannot write the shared Git metadata under the primary checkout's `.git` directory. The orchestrator must attach/adopt those worktrees from the host side before giving workers the full implementation prompt, and must act as the Git/queue bridge when workers report `Operation not permitted` on `.git/refs`, `.git/worktrees/*/index.lock`, or `.codex/tasks/*.tmp`. Workers should stop and report the exact blocked command instead of inventing alternate Git metadata workarounds.
