...
const LEGEND_WIDTH: float = 198.0
const LEGEND_PADDING: float = 12.0
const LEGEND_ROW_HEIGHT: float = 24.0
...
func _draw_map_legend() -> void:
	...
	draw_string(font, Vector2(legend_rect.position.x + LEGEND_PADDING, cursor_y + 9.0), "ROUTES", HORIZONTAL_ALIGNMENT_LEFT, legend_rect.size.x - LEGEND_PADDING * 2.0, 10, Color(0.82, 0.72, 0.57, 0.92))
	cursor_y += 17.0
	...
	for index: int in range(route_entries.size()):
		...
		var row: int = index / 2
		var center := Vector2(
			legend_rect.position.x + LEGEND_PADDING + float(column) * column_width + 9.0,
			cursor_y + float(row) * LEGEND_ROW_HEIGHT + 9.0
		)
		_draw_route_state_sample(center, str(entry.get("state", ROUTE_UNAVAILABLE)))
		draw_string(font, center + Vector2(14.0, 4.0), str(entry.get("label", "")), HORIZONTAL_ALIGNMENT_LEFT, column_width - 26.0, 9, Color("d9cbb2"))
	cursor_y += LEGEND_ROW_HEIGHT * 2.0 + LEGEND_SECTION_GAP
	draw_string(font, Vector2(legend_rect.position.x + LEGEND_PADDING, cursor_y + 9.0), "ROOMS", HORIZONTAL_ALIGNMENT_LEFT, legend_rect.size.x - LEGEND_PADDING * 2.0, 10, Color(0.82, 0.72, 0.57, 0.92))
	cursor_y += 17.0
	var icon_side: float = 18.0
	var icon_radius: float = 7.0
...
func _legend_rect() -> Rect2:
	...
	var height: float = LEGEND_PADDING * 2.0 + 34.0 + LEGEND_ROW_HEIGHT * 2.0 + LEGEND_SECTION_GAP + float(room_rows) * LEGEND_ROW_HEIGHT
	return Rect2(
