...
func _end_pointer_press() -> void:
	...
	if _drag_sound_pending:
		_play_click_feedback(false)
	elif _press_actionable or _drag_started:
		# The gesture already sounded on press/drag. Anchor the guard to its
		# release too, so holding the button cannot trigger a second click.
		_last_valid_gesture_msec = Time.get_ticks_msec()
	_left_pressed = false
