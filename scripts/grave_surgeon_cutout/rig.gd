...
## Reviewed Grave Surgeon topology and paint reuse the production skeleton loader.
const SurgeonMotion = preload("res://scripts/grave_surgeon_cutout/motion.gd")
const SURGEON_BASE: String = "res://assets/units/grave_surgeon_cutout"
var _paint_facing: String = ""
var _paint: Dictionary = {}
var _default_layers: Dictionary = {}

func _layout_path(which: String) -> String:
	return SURGEON_BASE.path_join(which + ".json")

func _resolve(path: String) -> String:
	return path if path.begins_with("res://") else SURGEON_BASE.path_join(path)

func apply_pose(clip_name: String, phase_value: float) -> void:
	var pose: Dictionary = SurgeonMotion.sample_pose(clip_name, phase_value, layout, facing)
	...
	if _paint_facing != facing:
		...
		for part_name: String in SurgeonMotion.sample_draw_order("walk", 0.0, layout, facing):
			...
			if matches.size() != 1:
				load_errors.append("Missing or ambiguous Grave Surgeon paint: " + part_name)
				continue
			_paint[part_name] = matches[0]
			_default_layers[part_name] = (matches[0] as CanvasItem).z_index
	var layers: Dictionary = SurgeonMotion.sample_draw_order(clip_name, phase_value, layout, facing)
