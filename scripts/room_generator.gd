...
func generate_room(run_seed: int, room: Dictionary, travel_dir: Vector2i) -> Dictionary:
	...
	if room_type == "campfire" or room_type == "treasure" or room_type == "scavenger" or room_type == "graftwright":
...
func _encounter_enemy_types(room_type: String, depth: int, rng: RandomNumberGenerator, room_element: String = ElementData.NONE, boss_id: String = "") -> Array:
	if room_type == "start" or room_type == "campfire" or room_type == "treasure" or room_type == "scavenger" or room_type == "graftwright":
		return []
	if room_type == "boss":
