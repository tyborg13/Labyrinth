...
# At the existing 1.18 body scale this preserves a slightly slower board pace
# than the protagonist while retaining the approved longer, planted stride.
...
func _ready() -> void:
	...
	for view: String in ["front", "rear"]:
		...
		if not rig.load_rig():
			push_error("Stone Warden cutout could not load: " + str(rig.load_errors))
		rigs[view] = rig
	_apply_pose()
...
static func uses_attack(effect: Dictionary, actor: Dictionary) -> bool:
	...
	return kind == "melee" or (kind == "aoe" and int(effect.get("range", 0)) <= 0)

static func attack_pose_phase(progress: float, contact: float) -> float:
	# Preserve the accepted preparation/contact/recovery poses. The authored
	# contact (0.55) lands at the existing melee or area result boundary.
...
static func attack_trail_phase(progress: float) -> float:
	# The existing slash is visible only during the mace's downstroke/contact,
	# so it cannot announce a hit while the Warden is still lifting the weapon.
