...
## One persistent canvas per Frostglass Lancer actor, shared by all retained board layers.
const Rig = preload("res://scripts/frostglass_lancer_cutout/rig.gd")
const EnemyFacing = preload("res://scripts/enemy_cutout_facing.gd")
const Motion = preload("res://scripts/frostglass_lancer_cutout/motion.gd")
const REST_PATH: String = "res://assets/units/frostglass_lancer_cutout/front/rest.png"
const SOURCE_SIZE := Vector2(255, 255)
const SOURCE_OFFSET := Vector2(128, 128)
const CANVAS_SIZE := Vector2i(512, 512)
const WALK_CYCLE_SECONDS: float = 0.40
...
func _ready() -> void:
	...
	viewport.name = "FrostglassCanvas"
	...
	for view: String in ["front", "rear"]:
		...
		if not rig.load_rig():
			push_error("Frostglass Lancer cutout could not load: " + str(rig.load_errors))
		rigs[view] = rig
	_apply_pose()
...
func present(motion: Dictionary, reduce: bool, enabled: bool = true) -> void:
	...
	if clip == "walk":
		...
		if phase >= 1.0:
			...
			clip = str(motion.get("action", "thrust"))
			if not clip in ["thrust", "cast", "pin"]:
				clip = "idle"
	else:
		clip = "idle"
...
func snapshot() -> Dictionary:
	return {"art": "frostglass_lancer_cutout_v01", "facing": facing, "mirrored": mirrored,
...
func source_socket(action: String, direction_delta: Vector2i) -> Vector2:
	var view: Dictionary = direction_for_delta(direction_delta)
	var rig: Node = rigs.get(str(view["facing"]), null) as Node
	if rig == null:
		return SOURCE_SIZE * 0.5
	var layout: Dictionary = rig.get("layout")
	var pose: Dictionary = Motion.sample_pose(action, 0.5, layout, str(view["facing"]))
	var tip: Vector2 = Motion.landmark(layout, "weapon_tip")
	var socket: Vector2 = Motion.world(pose, layout, "lance") * (tip - Motion.point(layout, "lance"))
	return Vector2(SOURCE_SIZE.x - socket.x, socket.y) if bool(view["mirrored"]) else socket
