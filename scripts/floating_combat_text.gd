...
static var _next_screen_layout_id: int = 0


# The source dictionary owns its presentation identity. Deep copies made by the
# action/timeline handoff retain it; independently created identical hits do not.
static func ensure_screen_layout_id(context: Dictionary) -> String:
	if str(context.get("screen_layout_id", "")).is_empty():
		_next_screen_layout_id += 1
		context["screen_layout_id"] = "popup:%d" % _next_screen_layout_id
	return str(context["screen_layout_id"])
...
static func animate_entries(base_entries: Array, elapsed_seconds: float, reduced_motion: bool) -> Array[Dictionary]:
	...
	for base_var: Variant in base_entries:
		...
		entry["screen_layout_key"] = ensure_screen_layout_id(base_entry)
...
static func timeline_group(base_entries: Array, start_seconds: float) -> Dictionary:
	for base_var: Variant in base_entries:
		if typeof(base_var) == TYPE_DICTIONARY:
			ensure_screen_layout_id(base_var as Dictionary)
	return {
...
static func animate_timeline(groups: Array, elapsed_seconds: float, reduced_motion: bool) -> Array[Dictionary]:
	...
	# The screen-space allocator places new arrivals around established lanes.
	# Restacking old batches here would jump them when a later action is queued.
	var animated: Array[Dictionary] = []
...
static func animate_entry(base_entry: Dictionary, progress: float, reduced_motion: bool) -> Dictionary:
	ensure_screen_layout_id(base_entry)
