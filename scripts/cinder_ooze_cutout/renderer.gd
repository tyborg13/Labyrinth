...
## One persistent canvas per Cinder Ooze, including its preview and death hold.
const Rig = preload("res://scripts/cinder_ooze_cutout/rig.gd")
const EnemyFacing = preload("res://scripts/enemy_cutout_facing.gd")
const Motion = preload("res://scripts/cinder_ooze_cutout/motion.gd")
const REST_PATH: String = "res://assets/units/cinder_ooze_cutout/front/rest.png"
const SOURCE_SIZE := Vector2(255,255)
const SOURCE_OFFSET := Vector2(128,128)
const CANVAS_SIZE := Vector2i(512,512)
const WALK_CYCLE_SECONDS: float = 0.62
const WALK_FRAME_SECONDS: float = 1.0/60.0
const IDLE_CYCLE_SECONDS: float = 1.8
const ATTACK_FRAMES: int = 42
const BLOOM_FRAMES: int = 48
...
func _ready() -> void:
	...
	viewport.name = "CinderOozeCanvas"
	...
	for view: String in ["front","rear"]:
		...
		if not rig.load_rig():
			push_error("Cinder Ooze cutout could not load: " + str(rig.load_errors))
		rigs[view] = rig
	_apply_pose()
...
static func uses_attack(effect: Dictionary, actor: Dictionary) -> bool:
	if str(actor.get("type","")) != "cinder_ooze": return false
	return str(effect.get("kind","")) == "melee" or (str(effect.get("kind","")) == "aoe" and int(effect.get("range",0)) <= 0)

static func action_clip(effect: Dictionary) -> String:
	return "bloom" if str(effect.get("kind","")) == "aoe" else "attack"

static func attack_frame_count(effect: Dictionary) -> int:
	return BLOOM_FRAMES if action_clip(effect) == "bloom" else ATTACK_FRAMES

static func attack_pose_phase(progress: float, contact: float, action: String = "attack") -> float:
	var boundary: float = clampf(contact,0.01,0.99)
	var authored: float = 0.38 if action == "bloom" else 0.42
	return authored * progress / boundary if progress <= boundary else authored + (1.0-authored)*(progress-boundary)/(1.0-boundary)
...
func present(motion: Dictionary, reduce: bool, enabled: bool = true) -> void:
	...
	if clip == "walk":
		...
		# Keep the common action-facing envelope; the creature's action selects
		# its own sampler without extending the shared facing policy.
		clip = str(motion.get("action","attack"))
		if phase >= 1.0:
			clip = "idle"
		else:
			phase = attack_pose_phase(phase,float(motion.get("contact",0.42)),clip)
	else:
		clip = "idle"
...
func _process(delta: float) -> void:
	if not active or not is_instance_valid(viewport): return
...
func _apply_pose() -> void:
	...
	if signature == _pose_signature: return
...
func snapshot() -> Dictionary:
	return {"art":"cinder_ooze_cutout_v01","facing":facing,"mirrored":mirrored,
