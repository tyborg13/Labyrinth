...
const ACTION_STEP_TRACKER_MIN_SIZE: Vector2 = Vector2(760.0, 108.0)
...
const ACTION_CONTEXT_MAX_WIDTH: float = 940.0
...
func _setup_action_step_tracker() -> void:
	...
	_action_step_tracker_title.custom_minimum_size = Vector2(174.0, 22.0)
	...
	_action_context_step_label.custom_minimum_size = Vector2(88.0, 22.0)
	...
	_action_context_target_label.custom_minimum_size = Vector2(108.0, 22.0)
	...
	_action_context_risk_panel.custom_minimum_size = Vector2(142.0, 24.0)
...
func _active_card_preview() -> Dictionary:
	if _drag_card_index >= 0:
		if bool(_drag_card_options.get("printed_playable", false)):
			return (_drag_card_options.get("play", {}) as Dictionary).duplicate(true)
		return {}
	if _selected_card_index >= 0:
