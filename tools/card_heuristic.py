...
generic damage/support bump. Chainbound Gaolers join depth 2-3 normal pools as
mid-slow pull/immobilize control anchors without stacking with wardens in their
seeded compositions. Later sequences keep the local curve while raising enemy HP
and intent baselines. Zekarion's 2x2 footprint makes printed reach feel larger,
so Tempest Breath is capped at range 3 after its one-tile advance to preserve
safe boss-room repositioning. Elemental combat rooms seed 2-3 traps across
eligible passable floor tiles, including the playable edge band, and those traps
blast adjacent tiles when stepped on or attacked. First-sequence standard trap
damage is 6/7/8 player-scale damage at depths 1/2/3, first-boss traps hit for 5
to avoid one-shotting full-health lightning wisps, and later sequences add 2
player-scale damage per completed sequence. Depth 1-2 fire rooms use shallow
burn on enemy attacks and fire traps before depth-3 fire/earth trap statuses cap
at 2. Every room places a 4 HP potion and 4 block shield, and combat rooms
scatter 5-7 low-HP boxes/crates across eligible passable floor tiles, including
edge-band tiles when connectivity stays intact. Those crates block movement
without blocking line of sight.
