...
def structure(s):
    ...
    expected = [dict(n, instrument='violin_1', pitch=n['pitch'] + 12) for n in notes(parent, 'grave_lead')]
    checks.require(notes(s, 'violin_1') == expected, 'Lead differs beyond documented octave and instrument move')
    checks.require(notes(s, 'drums') == notes(parent, 'drums'), 'Percussion events changed')
    ...
    for part in ['violin_1', 'violin_2', 'viola']:
        ...
        if part != 'violin_1':
            ...
            checks.require(len({n['pitch'] for n in sequence}) >= 7, 'Middle voice lacks movement')
        voices[part] = {'notes': len(sequence), 'gate_coverage': coverage,
